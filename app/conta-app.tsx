...
export default function ContaApp() {
  ...
  const viewCapability:Record<View,string>={pos:"pos.view",purchases:"purchases.view",expenses:"expenses.view",customers:"customers.view",suppliers:"suppliers.view",warehouses:"warehouses.view",transfers:"warehouses.transfer",adjustments:"warehouses.adjust",products:"products.view",records:"records.view",reports:"reports.view",banks:"banks.view",settings:"settings.view"};
  useEffect(() => {
...
const permissionRows:Array<{name:string;actions:Partial<Record<"view"|"create"|"edit"|"delete",string>>}>=[
 {name:"نقطة البيع",actions:{view:"pos.view",create:"pos.create",edit:"pos.edit",delete:"pos.delete"}},{name:"فواتير الشراء",actions:{view:"purchases.view",create:"purchases.create",edit:"purchases.edit",delete:"purchases.delete"}},{name:"سجل الفواتير",actions:{view:"records.view"}},{name:"مرتجع البيع",actions:{create:"returns.sale"}},{name:"مرتجع الشراء",actions:{create:"returns.purchase"}},{name:"المنتجات",actions:{view:"products.view",create:"products.create",edit:"products.edit",delete:"products.delete"}},{name:"العملاء",actions:{view:"customers.view",create:"customers.create",edit:"customers.edit"}},{name:"الموردون",actions:{view:"suppliers.view",create:"suppliers.create",edit:"suppliers.edit"}},{name:"حساب العميل / التحصيل",actions:{view:"customers.view",create:"customers.collect"}},{name:"حساب المورد / الدفع",actions:{view:"suppliers.view",create:"suppliers.pay"}},{name:"المخازن",actions:{view:"warehouses.view",create:"warehouses.create",edit:"warehouses.edit"}},{name:"جرد المخزن",actions:{view:"warehouses.inventory.view"}},{name:"التحويل بين المخازن",actions:{create:"warehouses.transfer"}},{name:"تصحيح المخزون",actions:{create:"warehouses.adjust"}},{name:"البنوك ووسائل الدفع",actions:{view:"banks.view",create:"banks.create",edit:"banks.edit",delete:"banks.delete"}},{name:"حركة الحسابات",actions:{view:"banks.movements.view"}},{name:"التحويلات البنكية",actions:{view:"banks.view",create:"banks.transfer"}},{name:"السحب والإيداع",actions:{view:"banks.view",create:"banks.deposit_withdraw"}},{name:"تصحيح رصيد بنك/وسيلة دفع",actions:{view:"banks.view",create:"banks.balance_correct"}},{name:"المصاريف",actions:{view:"expenses.view",create:"expenses.create",edit:"expenses.edit",delete:"expenses.delete"}},{name:"التقارير",actions:{view:"reports.view"}},{name:"الإعدادات",actions:{view:"settings.view"}},{name:"النسخ الاحتياطي والاستعادة",actions:{create:"settings.backup.manage"}},{name:"استيراد النظام السابق",actions:{create:"settings.legacy.import"}},{name:"المستخدمون والصلاحيات",actions:{create:"settings.users.manage"}},
];
function UsersPermissions(){const [users,setUsers]=useState<ManagedUser[]>([]),[editing,setEditing]=useState<ManagedUser|null>(null),[name,setName]=useState(""),[username,setUsername]=useState(""),[password,setPassword]=useState(""),[active,setActive]=useState(true),[permissions,setPermissions]=useState<string[]>([]),[error,setError]=useState("");const load=useCallback(()=>fetch("/api/settings/users").then(readApiResponse).then(v=>setUsers((v as {users:ManagedUser[]}).users)),[]);useEffect(()=>{void load()},[load]);const select=(u:ManagedUser|null)=>{setEditing(u);setName(u?.name??"");setUsername(u?.username??"");setPassword("");setActive(u?.isActive??true);setPermissions(u?.permissions??[])};const toggle=(key:string,on:boolean)=>setPermissions(p=>on?[...new Set([...p,key])]:p.filter(x=>x!==key));const save=async()=>{setError("");try{const response=await fetch(editing?`/api/settings/users/${editing.id}`:"/api/settings/users",{method:editing?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name,username,password,isActive:active,permissions})});await readApiResponse(response);await load();select(null)}catch(e){setError(e instanceof Error?e.message:"تعذر الحفظ")}};return <FramedSection title="المستخدمون والصلاحيات" className="users-permissions"><div className="users-list"><button className="primary" onClick={()=>select(null)}>إنشاء مستخدم</button><table className="erp-table"><thead><tr><th>رقم</th><th>الاسم</th><th>اسم المستخدم</th><th>الحالة</th><th>تعديل</th></tr></thead><tbody>{users.map((u,i)=><tr key={u.id}><td>{number(i+1)}</td><td>{u.name}</td><td dir="ltr">{u.username}</td><td>{u.isActive?"نشط":"معطل"}</td><td><button className="soft" onClick={()=>select(u)}>تعديل</button></td></tr>)}</tbody></table></div><div className="user-editor"><div className="user-fields"><label>الاسم<input value={name} onChange={e=>setName(e.target.value)}/></label><label>اسم المستخدم<input dir="ltr" value={username} onChange={e=>setUsername(e.target.value)}/></label><label>كلمة المرور<input type="password" placeholder={editing?"اتركها فارغة للاحتفاظ بالحالية":"8 أحرف على الأقل"} value={password} onChange={e=>setPassword(e.target.value)}/></label><label className="active-toggle"><input type="checkbox" checked={active} onChange={e=>setActive(e.target.checked)}/>{active?"المستخدم مفعل":"المستخدم معطل"}</label></div><div className="permissions-grid"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم الشاشة</th><th>عرض</th><th>إضافة</th><th>تعديل</th><th>حذف</th><th>تحكم كامل</th></tr></thead><tbody>{permissionRows.map((row,i)=>{const keys=Object.values(row.actions),full=keys.length>0&&keys.every(k=>permissions.includes(k));return <tr key={row.name}><td>{number(i+1)}</td><td>{row.name}</td>{(["view","create","edit","delete"] as const).map(action=>{const key=row.actions[action];return <td key={action}>{key?<input type="checkbox" checked={permissions.includes(key)} onChange={e=>toggle(key,e.target.checked)}/>:"—"}</td>})}<td><input type="checkbox" checked={full} onChange={e=>{if(e.target.checked)keys.forEach(k=>toggle(k,true))}}/></td></tr>})}</tbody></table></div>{error&&<div className="error">{error}</div>}<button className="primary" disabled={!name||!username||(!editing&&!password)} onClick={()=>void save()}>حفظ</button></div></FramedSection>}
function SettingsPage({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  const resume=async(run:ImportRun)=>{setBusy("import");setImportRun(run);setFailure("");try{await advance(run);await Promise.all([reload(),loadHistory()])}catch(e){setFailure(e instanceof Error?e.message:"تعذر استئناف العملية")}finally{setBusy("")}};
    ...
    {(data.principal.principalType==="owner"||data.principal.permissions.includes("settings.users.manage"))&&<UsersPermissions/>}
...
function Banks({ data, run }: { data: BootstrapData; run: RunCommand }) {
  ...
  const todayMovements = data.financialMovements.filter(m => m.occurredAt.slice(0, 10) === today && m.type !== "opening-balance" && m.type !== "balance-correction");
...
function PartyPage({party,data,close,openDoc,run}:{party:Party;data:BootstrapData;close:()=>void;openDoc:(id:string)=>void;run:RunCommand}) {
  ...
  const customer=resolvePartyType(party)==="customer", balance=customer?party.receivable:party.payable;
  ...
  return <section className={`party-detail ${customer?"customer-detail":"supplier-detail"}`}><button className="back" onClick={close}>← العودة إلى {customer?"العملاء":"الموردين"}</button><div className="party-detail-top"><FramedSection title={customer?"حساب العميل":"حساب المورد"} className="party-account-summary"><div className="party-identity"><div className="party-identity-field"><small>{customer?"اسم العميل":"اسم المورد"}</small><strong>{party.name}</strong></div><div className="party-identity-field"><small>الهاتف</small><strong className="party-phone"><bdi dir="ltr">{party.phone||"—"}</bdi></strong></div></div><div className={`party-balance ${balance===0?"neutral":customer?"receivable":"payable"}`}><span>{customer?"المستحق لنا":"المستحق علينا للمورد"}</span><strong>{number(balance)}</strong></div></FramedSection><FramedSection title={customer?"تسجيل تحصيل من العميل":"دفع للمورد"} className="party-payment-panel"><div className="party-payment-row"><label>حساب الدفع<select value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}>{data.paymentAccounts.filter(a=>a.isActive).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={amount} onChange={setAmount}/></label><button className="primary" disabled={!amount||val(amount)>balance} onClick={()=>void submit()}>{customer?"تسجيل التحصيل":"دفع للمورد"}</button></div></FramedSection></div><FramedSection title="سجل الفواتير / المعاملات" className="party-history"><div className="party-history-toolbar"><label>من<input type="date" dir="ltr" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>إلى<input type="date" dir="ltr" value={to} onChange={e=>setTo(e.target.value)}/></label></div><Recent title="الحركات" docs={docs} openDoc={openDoc} bare/></FramedSection></section>;
}
...
function Products({ data, run }: { data: BootstrapData; run: RunCommand }) {
  ...
  return <section className="workspace-page products-page">
    ...
    {formOpen && <div className="modal-overlay" role="dialog" aria-modal="true" aria-label={editing ? `تعديل ${editing.name}` : "إضافة منتج"}><div className="modal-card product-modal"><ProductForm run={run} product={editing} warehouses={data.warehouses} close={() => setFormOpen(false)} /></div></div>}
  </section>;
}
...
function ProductForm({ run, close, product, warehouses }: { run: RunCommand; close: () => void; product: Product | null; warehouses: BootstrapData["warehouses"] }) {
  ...
  return <form className="panel product-form" onSubmit={async event => { event.preventDefault(); const sensitive = product && (name.trim() !== product.name || (cost === "" ? null : val(cost)) !== product.pieceCost); const confirmed = sensitive ? window.confirm(`أنت تغيّر بيانات أساسية للمنتج «${product.name}». هل تريد المتابعة؟`) : true; if (!confirmed) return;
    await run({ type: product ? "product.update" : "product.create", id: product?.id, name, barcode, expiryDate, note, pieceCost: cost, piecePrice: price, wholesalePrice, openingStock: product ? undefined : openingStock, openingWarehouseId: product ? undefined : openingWarehouseId, confirmSensitive: confirmed }, product ? "تم تعديل المنتج" : "تم إنشاء المنتج"); close(); }}>
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const [allTimeActive,setAllTimeActive]=useState(false), [bankOpen,setBankOpen]=useState(false);
