...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  {tab==="transfers"&&<div className="bank-tab-content bank-tab-transfers"><FramedSection title="تحويل جديد" className="bank-operation"><form className="transfer-form" onSubmit={async e=>{e.preventDefault();await run({type:"account-transfer.post",fromAccountId:transferFrom,toAccountId:transferTo,amount:val(amount),note},"تم التحويل بين الحسابات");setAmount("");setNote("")}}><label>من الحساب<select required value={transferFrom} onChange={e=>setTransferFrom(e.target.value)}><option value="">اختر المصدر</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>إلى الحساب<select required value={transferTo} onChange={e=>setTransferTo(e.target.value)}><option value="">اختر الوجهة</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={amount} onChange={setAmount}/></label><label>ملاحظة<input value={note} onChange={e=>setNote(e.target.value)}/></label><button className="primary" disabled={!transferFrom||!transferTo||transferFrom===transferTo||!amount}>اعتماد التحويل</button></form></FramedSection><FramedSection title="سجل التحويلات" className="bank-history"><div className="bank-filters"><BankScopeControls scope={transferScope}/><select value={transferFromFilter} onChange={e=>setTransferFromFilter(e.target.value)}><option value="">كل المرسلين</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={transferToFilter} onChange={e=>setTransferToFilter(e.target.value)}><option value="">كل المستلمين</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></div><div className="erp-table-wrap transfer-list"><table className="erp-table"><thead><tr><th>التاريخ</th><th>من</th><th>إلى</th><th>المبلغ</th><th>المرجع</th></tr></thead><tbody>{transfers.map(t=><tr key={t.id} onClick={()=>setDetail({type:"تحويل",occurredAt:t.occurredAt,amount:t.amount,reference:t.number,note:t.note,from:name(t.fromAccountId),to:name(t.toAccountId)})}><td>{formatDateTime(t.occurredAt)}</td><td>{name(t.fromAccountId)}</td><td>{name(t.toAccountId)}</td><td>{money(t.amount)}</td><td dir="ltr">{t.number}</td></tr>)}</tbody></table></div></FramedSection></div>}
  {tab==="adjustment"&&<div className="bank-tab-content bank-tab-adjustment"><FramedSection title="عملية سحب أو إيداع" className="bank-operation"><form className="bank-adjustment-form" onSubmit={async e=>{e.preventDefault();await run({type:"account-adjustment.post",accountId:adjustmentAccount,direction:adjustmentDirection,amount:val(adjustmentAmount),note:adjustmentNote},adjustmentDirection==="deposit"?"تم الإيداع":"تم السحب");setAdjustmentAmount("");setAdjustmentNote("")}}><div className="bank-adjustment-direction"><button type="button" className="selection-option" aria-pressed={adjustmentDirection==="deposit"} onClick={()=>setAdjustmentDirection("deposit")}>إيداع</button><button type="button" className="selection-option" aria-pressed={adjustmentDirection==="withdrawal"} onClick={()=>setAdjustmentDirection("withdrawal")}>سحب</button></div><label>الحساب<select required value={adjustmentAccount} onChange={e=>setAdjustmentAccount(e.target.value)}><option value="">اختر الحساب</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={adjustmentAmount} onChange={setAdjustmentAmount}/></label><label>ملاحظة<input value={adjustmentNote} onChange={e=>setAdjustmentNote(e.target.value)}/></label><button className="primary">اعتماد العملية</button></form></FramedSection><FramedSection title="سجل السحب والإيداع" className="bank-history"><div className="bank-filters"><BankScopeControls scope={adjustmentScope}/><select value={adjustmentFilter} onChange={e=>setAdjustmentFilter(e.target.value)}><option value="">كل الحسابات</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={adjustmentType} onChange={e=>setAdjustmentType(e.target.value)}><option value="">إيداع وسحب</option><option value="manual-deposit">إيداع</option><option value="manual-withdrawal">سحب</option></select></div><div className="erp-table-wrap adjustment-list"><table className="erp-table"><thead><tr><th>المرجع</th><th>التاريخ</th><th>الحساب</th><th>النوع</th><th>المبلغ</th><th>الملاحظة</th></tr></thead><tbody>{adjustments.map(m=><tr key={m.id} onClick={()=>inspectMovement(m)}><td dir="ltr">{m.documentNumber}</td><td>{formatDateTime(m.occurredAt)}</td><td>{name(m.paymentMethod)}</td><td>{movementLabels[m.type]}</td><td>{money(m.amount)}</td><td>{m.note||"—"}</td></tr>)}</tbody></table></div></FramedSection></div>}
...
function Warehouses({ data, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  const [draftFrom,setDraftFrom]=useState(""),[draftTo,setDraftTo]=useState(""),[committedPeriod,setCommittedPeriod]=useState<CommittedPeriod>(null),[hasInventoryView,setHasInventoryView]=useState(false),[periodError,setPeriodError]=useState("");
  ...
  const showAllInventory=()=>{setCommittedPeriod(null);setHasInventoryView(true);setPeriodError("")};
