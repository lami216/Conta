...
function SearchableSelect({ value, onChange, options, placeholder, searchPlaceholder, disabled = false, allowEmpty = false, floating = false, variant = "normal", searchable = true, ariaLabel }: {
  value: string; onChange: (value: string) => void; options: SelectOption[];
  placeholder: string; searchPlaceholder?: string; disabled?: boolean; allowEmpty?: boolean; floating?: boolean; variant?: "normal" | "compact" | "pos-native"; searchable?: boolean; ariaLabel?: string;
}) {
  ...
  const position = useCallback(() => {
    ...
    const rect = root.current.getBoundingClientRect(), margin = 8, posNative = variant === "pos-native", desiredHeight = Math.min(variant === "normal" ? 330 : 235, window.innerHeight - margin * 2);
    const below = window.innerHeight - rect.bottom - margin, above = rect.top - margin, opensUp = below < 220 && above > below;
    const width = posNative ? Math.min(rect.width, window.innerWidth - margin * 2) : Math.min(Math.max(rect.width, variant === "compact" ? 220 : 280), variant === "compact" ? 300 : window.innerWidth - margin * 2, window.innerWidth - margin * 2);
  ...
  const list = <div ref={popover} className={`combobox-popover${floating ? " combobox-popover-floating" : ""}${variant === "compact" ? " combobox-popover-compact" : ""}${variant === "pos-native" ? " combobox-popover-pos-native" : ""}`} style={floating ? floatingStyle : undefined}>
    {searchable && <label className="search"><Search /><input autoFocus value={query} placeholder={searchPlaceholder} onChange={e => { setQuery(e.target.value); setActive(0); }} onKeyDown={e => {
  ...
  return <div className={`combobox${variant === "compact" ? " combobox-compact" : ""}${variant === "pos-native" ? " combobox-pos-native" : ""}`} ref={root}>
...
function Pos({
  ...
  const customerSelect = <SearchableSelect value={partyId} onChange={setPartyId} placeholder={payment === "note" ? "اختر العميل" : "بيع مباشر"} searchPlaceholder="ابحث باسم أو هاتف" floating allowEmpty={payment !== "note"} variant="pos-native" ariaLabel="العميل" options={data.parties.filter(p => resolvePartyType(p) === "customer").map(p => ({ value: p.id, label: p.name, search: p.phone }))} />;
...
function CompactPaymentSelector({ accounts, value, onChange }: { accounts: PaymentAccount[]; value: string; onChange: (id: string) => void }) {
  ...
  return <div className="compact-payment" aria-label="طريقة الدفع">
    ...
    <div className={selectedBank ? "bank-option selected-bank" : "bank-option"}><SearchableSelect value={selectedBank?.id ?? ""} onChange={next => next && onChange(next)} placeholder="بنك" searchable={false} floating variant="pos-native" ariaLabel="الحساب البنكي" options={banks.map(account => ({ value: account.id, label: account.name }))} /></div>
  </div>;
}
  ...
  return <section className="workspace-page products-page">
    ...
    <FramedSection title="قائمة المنتجات" className="scroll-panel product-management">
      ...
      <table className="erp-table" aria-label="كل المنتجات"><colgroup><col style={{width:"10%"}}/><col style={{width:"22%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"12%"}}/><col style={{width:"23%"}}/></colgroup><thead><tr>
        <th>م</th><th>الاسم</th><th>{sortHeader("price", "سعر البيع")}</th><th>سعر الجملة</th><th>{sortHeader("cost", "آخر شراء")}</th><th>{sortHeader("stock", "المخزون")}</th><th>إجراءات</th>
      </tr></thead><tbody>
