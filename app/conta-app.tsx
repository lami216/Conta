...
function useBankScope(){const today=localBusinessDay(),[draftFrom,setDraftFrom]=useState(today),[draftTo,setDraftTo]=useState(today),[period,setPeriod]=useState<CommittedPeriod>(()=>({from:today,to:today}));return {draftFrom,draftTo,setDraftFrom,setDraftTo,period,commit:()=>setPeriod({from:draftFrom,to:draftTo}),all:()=>setPeriod(null)}}
...
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    ...
    <div className="party-list-metrics"><span className="metric-neutral"><small>{customer?"إجمالي مبيعات العملاء":"إجمالي مشترياتنا من الموردين"}</small><MoneyValue value={total} className="financial-amount-summary"/></span>{customer&&<span className={Number(profit??0)>0?"metric-positive":Number(profit??0)<0?"metric-negative":"metric-neutral"}><small>إجمالي الربح من مبيعات العملاء</small><MoneyValue value={profit??0} tone={Number(profit??0)>0?"positive":Number(profit??0)<0?"negative":"neutral"} className="financial-amount-summary"/></span>}</div>
  </section>;
}
