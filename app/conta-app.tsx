...
type FilePreview = { format:string;uploadId?:string;schemaVersion?:number;createdAt?:string;counts?:Record<string,number>;groups?:Array<{key:string;label:string;count:number;status:string}>;warnings?:string[] };
type ImportRun = {importRunId:string;state:string;phase:string;progress?:{processed:number;total:number;label:string};counts?:Record<string,{processed:number;created:number;existing:number;skipped:number}>;reviewCount?:number;publicError?:string};
function SettingsPage({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  const [nativeFile,setNativeFile]=useState<File|null>(null),[nativePreview,setNativePreview]=useState<FilePreview|null>(null),[legacyPreview,setLegacyPreview]=useState<FilePreview|null>(null),[importRun,setImportRun]=useState<ImportRun|null>(null),[busy,setBusy]=useState(""),[message,setMessage]=useState(""),[failure,setFailure]=useState("");
  ...
  const chooseLegacy=async(file:File|null)=>{setLegacyPreview(null);setImportRun(null);setFailure("");if(!file)return;setBusy("legacy");try{setLegacyPreview(await uploadLegacy(file) as FilePreview);}catch(e){setFailure(e instanceof Error?e.message:"قاعدة غير صالحة");}finally{setBusy("");}};
  ...
  const importLegacy=async()=>{if(!legacyPreview?.uploadId)return;setBusy("import");setFailure("");try{let run=await readApiResponse(await fetch("/api/settings/legacy/upload/complete",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({uploadId:legacyPreview.uploadId,action:"import",stockPolicy:"current"})})) as ImportRun;setImportRun(run);while(run.state!=="completed"&&run.state!=="failed"){await new Promise(resolve=>setTimeout(resolve,400));run=await readApiResponse(await fetch(`/api/settings/legacy/import-runs/${encodeURIComponent(run.importRunId)}/advance`,{method:"POST",headers:{"content-type":"application/json"},body:"{}"})) as ImportRun;setImportRun(run)}if(run.state==="failed")throw new Error(`${run.publicError} (${run.importRunId})`);setMessage("تم استيراد البيانات بنجاح. المخزون الحالي محفوظ عند التعارض");await reload();}catch(e){setFailure(e instanceof Error?e.message:"تعذر الاستيراد");}finally{setBusy("");}};
  ...
  return <section className="settings-page"><div className="settings-grid">
    ...
    <article className="panel settings-card"><h2>استيراد من نظام سابق</h2><p>عملية دمج آمنة؛ المخزون الحالي محفوظ عند التعارض.</p><label className="file-button">اختيار ملف SQLite<input type="file" accept=".db,.sqlite,application/vnd.sqlite3" onChange={e=>void chooseLegacy(e.target.files?.[0]??null)}/></label>{busy==="legacy"&&<small>جاري فحص الجداول…</small>}{legacyPreview&&<div className="preview-box">{(["ready","unsupported","review"] as const).map(status=><div key={status}><strong>{status==="ready"?"سيتم استيراده":status==="review"?"بيانات مالية تحتاج مراجعة":"غير مطلوب في Conta"}</strong>{legacyPreview.groups?.filter(g=>g.status===status).map(g=><span key={g.key}>{g.label} <b>{number(g.count)}</b></span>)}</div>)}</div>}{importRun&&<div className="preview-box"><strong>{importRun.state==="completed"?"تم الاستيراد بنجاح":importRun.progress?.total?`جاري استيراد ${importRun.progress.label}…`:"جاري تجهيز عملية الاستيراد…"}</strong>{!!importRun.progress?.total&&<span>{importRun.progress.label}: {number(importRun.progress.processed)} / {number(importRun.progress.total)}</span>}{importRun.state==="completed"&&Object.entries(importRun.counts??{}).map(([key,value])=><small key={key}>{value.processed} سجل — {value.created} جديد، {value.existing} موجود مسبقًا</small>)}</div>}<button className="primary" disabled={!legacyPreview||!!busy} onClick={()=>void importLegacy()}>{busy==="import"?"جاري الاستيراد…":"استيراد البيانات"}</button></article>
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const display=(key:string,value:unknown)=>{if(value===null&&(key==="cost"||key==="profit"||key==="margin"))return "—";if(key==="paymentMethod")return accountName(value);if(key==="movementType")return movementLabels[String(value)]??"عملية غير معروفة";if(key==="occurredAt"||key==="lastMovement")return value?formatDateTime(String(value)):"—";if(key==="date")return value?formatDate(String(value)):"—";if(typeof value==="number")return number(value);if(typeof value==="boolean")return value?"متكرر":"مرة واحدة";return String(value??"—")};
  const summaryValue=(key:string,value:unknown)=>hasUnknownCost&&["cost","profit"].includes(key)?"—":typeof value==="number"?number(Number(value)):String(value);
