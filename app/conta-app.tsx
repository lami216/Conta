...
function SettingsPage({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  const importLegacy=async()=>{if(!legacyPreview?.uploadId)return;setBusy("import");setFailure("");try{let run=await readApiResponse(await fetch("/api/settings/legacy/upload/complete",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({uploadId:legacyPreview.uploadId,action:"import",stockPolicy:"current"})})) as ImportRun;setImportRun(run);while(run.state!=="completed"&&run.state!=="failed"){await new Promise(resolve=>setTimeout(resolve,1200));run=await readApiResponse(await fetch(`/api/settings/legacy/import-runs/${encodeURIComponent(run.importRunId)}`)) as ImportRun;setImportRun(run)}if(run.state==="failed")throw new Error(`${run.publicError} (${run.importRunId})`);setMessage("تم استيراد البيانات بنجاح. المخزون الحالي محفوظ عند التعارض");await reload();}catch(e){setFailure(e instanceof Error?e.message:"تعذر الاستيراد");}finally{setBusy("");}};
