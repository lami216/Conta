...
import { detectPermissionPreset, permissionPresets, permissionRows, presetLabels, setPermission, setRowFullControl, type AccountPreset } from "./user-permissions";
...
function UsersPermissions() {
  ...
  const [name,setName]=useState(""),[username,setUsername]=useState(""),[password,setPassword]=useState(""),[active,setActive]=useState(true),[permissions,setPermissions]=useState<string[]>([]),[preset,setPreset]=useState<AccountPreset>("custom");
  ...
  const select=useCallback((user:ManagedUser|null)=>{setEditing(user);setName(user?.name??"");setUsername(user?.username??"");setPassword("");setActive(user?.isActive??true);setPermissions(user?.permissions??[]);setPreset(user?detectPermissionPreset(user.permissions):"custom");setError("");setSuccess("")},[]);
  ...
  const changePermissions=(next:string[])=>{setPermissions(next);setPreset(detectPermissionPreset(next))};
  const choosePreset=(next:AccountPreset)=>{setPreset(next);if(next!=="custom")setPermissions([...permissionPresets[next]])};
  const save=async()=>{setSaving(true);setError("");setSuccess("");try{const isEditing=!!editing;const response=await fetch(isEditing?`/api/settings/users/${editing.id}`:"/api/settings/users",{method:isEditing?"PUT":"POST",headers:{"content-type":"application/json"},body:JSON.stringify({name,username,password,isActive:active,permissions})});const result=await readApiResponse(response) as {user?:ManagedUser};const selectedId=editing?.id??result.user?.id;if(!selectedId)throw new Error("تم الحفظ لكن تعذر تحديد المستخدم");const refreshed=await load(selectedId);if(!refreshed)throw new Error("تم الحفظ، لكن تعذر تحديث قائمة المستخدمين");setSuccess(isEditing?"تم حفظ تعديلات المستخدم":"تم إنشاء المستخدم بنجاح")}catch(reason){setError(reason instanceof Error?reason.message:"تعذر الحفظ")}finally{setSaving(false)}};
  return <FramedSection title="المستخدمون والصلاحيات" className="users-permissions">
    <div className="users-permissions-layout">
      <fieldset className="permissions-pane"><legend>صلاحيات المستخدم</legend><div className="permissions-grid"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم الشاشة</th><th>عرض</th><th>إضافة</th><th>تعديل</th><th>حذف</th><th>تحكم كامل</th></tr></thead><tbody>{permissionRows.map((row,i)=>{const keys=Object.values(row.actions),full=keys.length>0&&keys.every(key=>permissions.includes(key));return <tr key={row.name}><td>{number(i+1)}</td><td>{row.name}</td>{(["view","create","edit","delete"] as const).map(action=>{const key=row.actions[action];return <td key={action}>{key?<input aria-label={`${row.name} ${action}`} type="checkbox" checked={permissions.includes(key)} onChange={event=>changePermissions(setPermission(permissions,key,event.target.checked))}/>:<span className="permission-unavailable">—</span>}</td>})}<td><input aria-label={`${row.name} تحكم كامل`} type="checkbox" checked={full} onChange={event=>changePermissions(setRowFullControl(permissions,keys,event.target.checked))}/></td></tr>})}</tbody></table></div></fieldset>
      <aside className="user-admin-pane">
        <fieldset className="user-details-form"><legend>بيانات المستخدم</legend><div className="user-mode"><strong>{editing?"تعديل المستخدم":"مستخدم جديد"}</strong>{editing&&<span>{editing.name}</span>}</div><div className="user-fields"><label>الاسم<input value={name} required onChange={event=>setName(event.target.value)}/></label><label>اسم المستخدم<input dir="ltr" value={username} required autoComplete="off" onChange={event=>setUsername(event.target.value)}/></label><label>كلمة المرور<input type="password" value={password} minLength={8} autoComplete="new-password" placeholder={editing?"اتركها فارغة للاحتفاظ بكلمة المرور الحالية":"8 أحرف على الأقل"} onChange={event=>setPassword(event.target.value)}/></label><label>نوع الحساب<select value={preset} onChange={event=>choosePreset(event.target.value as AccountPreset)}>{Object.entries(presetLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label></div><label className="active-toggle"><input type="checkbox" checked={active} onChange={event=>setActive(event.target.checked)}/><span>الحساب مفعل</span><b>{active?"مفعل":"معطل"}</b></label><div className="user-actions"><button className="soft" type="button" onClick={()=>select(null)}>مستخدم جديد</button><button className="primary" type="button" disabled={saving||!name.trim()||!username.trim()||(!editing&&password.length<8)||(!!password&&password.length<8)} onClick={()=>void save()}>{saving?"جاري الحفظ…":"حفظ المستخدم"}</button>{editing&&<button className={active?"danger":"soft"} type="button" onClick={()=>setActive(value=>!value)}>{active?"تعطيل المستخدم":"تفعيل المستخدم"}</button>}</div>{error&&<div className="error" role="alert">{error}</div>}{success&&<div className="success" role="status">{success}</div>}</fieldset>
        <fieldset className="users-list"><legend>قائمة المستخدمين</legend><div className="users-list-scroll"><table className="erp-table"><thead><tr><th>رقم</th><th>الاسم</th><th>اسم المستخدم</th><th>النوع</th><th>الحالة</th><th>تعديل</th></tr></thead><tbody>{loading?<tr><td colSpan={6}>جاري تحميل المستخدمين…</td></tr>:loadError?<tr className="users-load-error"><td colSpan={6}>تعذر تحميل المستخدمين: {loadError} <button className="soft" onClick={()=>void load()}>إعادة المحاولة</button></td></tr>:users.length===0?<tr><td colSpan={6}>لا يوجد مستخدمون بعد</td></tr>:users.map((user,i)=><tr className={editing?.id===user.id?"selected":""} key={user.id}><td>{number(i+1)}</td><td>{user.name}</td><td dir="ltr">{user.username}</td><td>{presetLabels[detectPermissionPreset(user.permissions)]}</td><td><span className={user.isActive?"user-status active":"user-status"}>{user.isActive?"مفعل":"معطل"}</span></td><td><button className="soft" onClick={()=>select(user)}>تعديل</button></td></tr>)}</tbody></table></div></fieldset>
      </aside>
    </div>
  </FramedSection>
}
