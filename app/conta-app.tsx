...
const empty: BootstrapData = {
  ...
  branding: { storeName:"Conta",storePhone:"",storeAddress:"",registrationNumber:"",taxNumber:"",footerNote:"",nameFont:"tahoma",nameFontSize:24,nameFontWeight:800 },
  generalSettings: { hideFinancialAmountsByDefault: true },
  nextProductCode: 1,
...
const FinancialPrivacyContext = createContext(true);
export type MoneyTone = "positive" | "negative" | "neutral";
export function PrivateMoney({value,tone="neutral",className=""}:{value:number;tone?:MoneyTone;className?:string}) {
  const hideByDefault=useContext(FinancialPrivacyContext),[revealed,setRevealed]=useState(()=>!hideByDefault),label=revealed?"إخفاء المبلغ":"إظهار المبلغ";
  return <><span className={`private-money no-print ${className}`.trim()}><span className={`private-money-value financial-amount ${revealed?`money-${tone}`:"money-masked"}`} aria-label={revealed?undefined:"المبلغ مخفي"} aria-live="polite">{revealed?money(value):"****** MRU"}</span><button type="button" className="private-money-toggle" aria-label={label} title={label} onClick={()=>setRevealed(value=>!value)}>{revealed?<EyeOff aria-hidden="true"/>:<Eye aria-hidden="true"/>}</button></span><span className="financial-amount print-only">{money(value)}</span></>;
}
...
export default function ContaApp() {
  ...
  return (
    <FinancialPrivacyContext.Provider value={data.generalSettings.hideFinancialAmountsByDefault}>
...
function GeneralSettings({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  const [branding,setBranding]=useState(data.branding),[privacy,setPrivacy]=useState(data.generalSettings),[saving,setSaving]=useState(false),[notice,setNotice]=useState("");
  const canBrand=data.principal.principalType==="owner"||data.principal.permissions.includes("settings.branding.manage"),canPrivacy=data.principal.principalType==="owner"||data.principal.permissions.includes("settings.general.manage");
...
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    ...
    <FramedSection title={plural} className="parties-list"><div className="erp-table-wrap party-grid"><table className="erp-table" aria-label={plural}><thead><tr><th>رقم</th><th>اسم {noun}</th><th>الهاتف</th><th>{customer?"إجمالي مشترياته":"إجمالي مشترياتنا منه"}</th>{customer&&<th>الربح الإجمالي</th>}<th>الرصيد الحالي</th><th>إجراء</th></tr></thead><tbody>{list.map((party,index)=>{const balance=party.receivable-party.payable,metric=metricMap.get(party.id)!;return <tr key={party.id} onClick={()=>openParty(party)}><td className="num-cell">{number(index+1)}</td><td className="name-cell">{party.name}</td><td dir="ltr">{party.phone||"—"}</td><td className="num-cell"><PrivateMoney value={metric.total}/></td>{customer&&<td className={`num-cell ${Number(metric.grossProfit??0)>0?"metric-positive":Number(metric.grossProfit??0)<0?"metric-negative":"metric-neutral"}`}><PrivateMoney value={metric.grossProfit??0} tone={Number(metric.grossProfit??0)>0?"positive":Number(metric.grossProfit??0)<0?"negative":"neutral"}/></td>}<td className={`num-cell ${balance>0?"positive":balance<0?"negative":""}`}><PrivateMoney value={Math.abs(balance)} tone={balance>0?"positive":balance<0?"negative":"neutral"}/></td><td className="action-cell"><button className="soft" onClick={event=>{event.stopPropagation();openParty(party)}}>كشف الحساب</button></td></tr>})}{!list.length&&<tr><td colSpan={customer?7:6}>{customer?"لا يوجد عملاء حتى الآن":"لا يوجد موردون"}</td></tr>}</tbody></table></div></FramedSection>
    <div className="party-list-metrics"><span className="metric-neutral"><small>{customer?"إجمالي مبيعات العملاء":"إجمالي مشترياتنا من الموردين"}</small><PrivateMoney value={total} className="private-money-summary"/></span>{customer&&<span className={Number(profit??0)>0?"metric-positive":Number(profit??0)<0?"metric-negative":"metric-neutral"}><small>إجمالي الربح من مبيعات العملاء</small><PrivateMoney value={profit??0} tone={Number(profit??0)>0?"positive":Number(profit??0)<0?"negative":"neutral"} className="private-money-summary"/></span>}</div>
  </section>;
}
function PartyPage({party,data,openDoc,run}:{party:Party;data:BootstrapData;openDoc:(id:string)=>void;run:RunCommand}) {
  const [from,setFrom]=useState(""),[to,setTo]=useState(""),[amount,setAmount]=useState(""),[paymentMethod,setPaymentMethod]=useState(""),[direction,setDirection]=useState<"receive"|"pay">("receive"),[note,setNote]=useState("");
...
function OfficialRecordSheet({presentation,branding}:{presentation:OfficialPresentation;branding:InvoiceBrandingSettings}){const style={fontFamily:invoiceFontFamilies[branding.nameFont],fontSize:`${branding.nameFontSize}pt`,fontWeight:branding.nameFontWeight} as CSSProperties,businessLine=[branding.storeAddress,branding.storePhone].filter(Boolean).join(" · "),registrationLine=[branding.registrationNumber&&`السجل التجاري: ${branding.registrationNumber}`,branding.taxNumber&&`الرقم الضريبي: ${branding.taxNumber}`].filter(Boolean).join(" · ");return <article className="official-record-sheet"><header className="official-record-header"><strong style={style}>{branding.storeName}</strong>{businessLine&&<span className="official-business-meta">{businessLine}</span>}{registrationLine&&<span className="official-business-meta">{registrationLine}</span>}<h1>{presentation.title}</h1><span>{presentation.meta.slice(0,2).map(x=>x[1]).join(" · ")}</span></header><div className="official-record-meta">{presentation.meta.slice(2).map(([label,value])=><span key={label}><small>{label}</small><b>{value}</b></span>)}</div>{presentation.columns&&<div className="official-record-table-wrap"><table className="official-record-table"><thead><tr>{presentation.columns.map(c=><th key={c}>{c}</th>)}</tr></thead><tbody>{presentation.rows?.map((row,i)=><tr key={i}>{row.map((v,j)=><td key={j}>{v}</td>)}</tr>)}</tbody></table></div>}{presentation.totals&&<div className={`official-record-totals ${presentation.tone??"neutral"}`}>{presentation.totals.map(([label,value])=><span key={label}><small>{label}</small><strong>{value}</strong></span>)}</div>}<footer>{branding.footerNote&&<span>{branding.footerNote}</span>}<small>تم إنشاء هذا المستند بواسطة Conta</small></footer></article>}
