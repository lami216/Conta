...
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    ...
    <FramedSection title={plural} className="parties-list"><div className="erp-table-wrap party-grid"><table className="erp-table" aria-label={plural}><thead><tr><th>رقم</th><th>اسم {noun}</th><th>الهاتف</th><th>الرصيد</th><th>إجراء</th></tr></thead><tbody>{list.map((party,index)=><tr key={party.id} onClick={()=>openParty(party)}><td className="num-cell">{number(index+1)}</td><td className="name-cell">{party.name}</td><td dir="ltr">{party.phone||"—"}</td><td className="num-cell">{party.receivable-party.payable>0?`${money(party.receivable-party.payable)} لنا`:party.receivable-party.payable<0?`${money(Math.abs(party.receivable-party.payable))} علينا`:"0"}</td><td className="action-cell"><button className="soft" onClick={event=>{event.stopPropagation();openParty(party)}}>كشف الحساب</button></td></tr>)}{!list.length&&<tr><td colSpan={5}>{customer?"لا يوجد عملاء حتى الآن":"لا يوجد موردون"}</td></tr>}</tbody></table></div></FramedSection>
  </section>;
}
function PartyPage({party,data,close,openDoc,run}:{party:Party;data:BootstrapData;close:()=>void;openDoc:(id:string)=>void;run:RunCommand}) {
  ...
  const customer=resolvePartyType(party)==="customer", balance=party.receivable-party.payable;
  ...
  const after=balance+(direction==="receive"?-val(amount):val(amount)); const balanceText=(value:number)=>value>0?`مستحق لنا ${number(value)}`:value<0?`مستحق علينا ${number(Math.abs(value))}`:"الحساب متوازن";
  ...
  return <section className={`party-detail ${customer?"customer-detail":"supplier-detail"}`}><button className="back" onClick={close}>← العودة إلى {customer?"العملاء":"الموردين"}</button><div className="party-detail-top"><FramedSection title={customer?"حساب العميل":"حساب المورد"} className="party-account-summary"><div className="party-identity"><div className="party-identity-field"><small>الاسم</small><strong>{party.name}</strong></div><div className="party-identity-field"><small>الهاتف</small><strong><bdi dir="ltr">{party.phone||"—"}</bdi></strong></div></div><div className={`party-balance ${balance>0?"receivable":balance<0?"payable":"neutral"}`}><span>{balance>0?"المستحق لنا":balance<0?"المستحق علينا":"الحساب متوازن"}</span><strong>{number(Math.abs(balance))}</strong></div></FramedSection><FramedSection title="حركة مالية" className="party-payment-panel"><div className="party-cash-direction"><button className="selection-option" aria-pressed={direction==="receive"} onClick={()=>setDirection("receive")}>استلام من {customer?"العميل":"المورد"}</button><button className="selection-option" aria-pressed={direction==="pay"} onClick={()=>setDirection("pay")}>دفع لـ{customer?"لعميل":"لمورد"}</button></div><div className="party-payment-row"><label>حساب الدفع<select value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}>{data.paymentAccounts.filter(a=>a.isActive).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={amount} onChange={setAmount}/></label><label>ملاحظة<input value={note} onChange={e=>setNote(e.target.value)}/></label><div className="party-balance-preview"><small>الرصيد الحالي: {balanceText(balance)}</small><small>بعد الحركة: {balanceText(after)}</small></div><button className="primary" disabled={!Number.isFinite(val(amount))||val(amount)<=0} onClick={()=>void submit()}>تسجيل الحركة</button></div></FramedSection></div><FramedSection title="سجل الحساب" className="party-history"><div className="party-history-toolbar"><label>من<input type="date" dir="ltr" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>إلى<input type="date" dir="ltr" value={to} onChange={e=>setTo(e.target.value)}/></label></div><Recent title="الحركات" docs={docs} openDoc={openDoc} bare/></FramedSection></section>;
}
...
const reportNames: Record<ReportType,string> = { overview:"التقرير الشامل",sales:"حركة المبيعات",purchases:"حركة المشتريات","product-sales":"تحليل مبيعات الأصناف",stock:"حركة المخزون",profit:"تحليل الأرباح",debts:"الحسابات والديون","party-ledger":"كشف حسابات الأطراف",financial:"الحركة المالية",expenses:"المصاريف",returns:"المرتجعات" };
