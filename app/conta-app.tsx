...
function Pos({
  ...
  const invoice = <FramedSection title="الفاتورة" className="invoice-card workspace-invoice">
    <InvoiceEditorToolbar number={editingDocument ? displayDocumentNumber(editingDocument) : String(data.nextDocumentSequences.sale)} newInvoice={() => { if (!dirty() || confirm("لديك تغييرات غير محفوظة. هل تريد بدء فاتورة جديدة؟")) resetEditor(); }}><button type="button" className="selection-option" aria-pressed={priceMode === "retail"} onClick={() => changePriceMode("retail")}>بيع الفرد</button><button type="button" className="selection-option" aria-pressed={priceMode === "wholesale"} onClick={() => changePriceMode("wholesale")}>بيع الجملة</button><BarcodeScanner products={data.products} onScan={add} enabled={scannerEnabled} onEnabledChange={setScannerEnabled} onError={setStockNotice} /></InvoiceEditorToolbar>
...
function InvoiceEditorToolbar({ number: invoiceNumber, newInvoice, children }: { number: string; newInvoice: () => void; children?: ReactNode }) {
  return <div className="invoice-lifecycle"><button type="button" className="invoice-new-button" onClick={newInvoice}>فاتورة جديدة</button><span className="invoice-number">رقم <b>{invoiceNumber}</b></span>{children}</div>;
}
