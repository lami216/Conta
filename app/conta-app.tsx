...
import { reportDateQuery, reportNumber, reportSummaryTone, reportTableModel, type ReportResponse, type ReportType } from "./report-types";
...
const reportOrder: ReportType[] = ["sales", "purchases", "product-sales", "returns", "stock", "debts", "party-ledger", "financial", "expenses", "overview"];
...
export default function ContaApp() {
  ...
  async function run(body: Record<string, unknown>, message: string, afterSuccess?: () => void) {
    ...
    await finishSuccessfulCommand(afterSuccess, () => reload({ blocking: false }));
    ...
    <div className={`app-shell section-${view}`} dir="rtl">
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label="التنقل الرئيسي">
          {nav.slice(0, 1).filter(n=>can(viewCapability[n.id])).map((n) => (
            <button
              key={n.id}
          ...
          {invoiceNav.some(n=>can(viewCapability[n.id]))&&<div className="nav-menu" ref={invoiceMenuRef}>
          ...
          {nav.slice(1).filter(n=>n.id!=="reports"&&n.id!=="settings"&&can(viewCapability[n.id])).map(n=><button key={n.id} className={view===n.id?"nav active":"nav"} onClick={()=>navigate(n.id)}><n.icon/><span>{n.label}</span></button>)}
          ...
          {nav.filter(n => n.id === "settings" && can("settings.view")).map(n => <button key={n.id} className={view === n.id ? "nav active" : "nav"} onClick={() => navigate(n.id)}><n.icon /><span>{n.label}</span></button>)}
        </nav>
        ...
        <div className="content">
          ...
          {loading ? (
            ...
            <>
              ...
              {view === "banks" && <Banks data={data} run={run} openDoc={openDoc} />}{" "}
...
function UsersPermissions({utilities}:{utilities:ReactNode}) {
...
function SettingsPage({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  const canManageUsers=data.principal.principalType==="owner"||data.principal.permissions.includes("settings.users.manage");
  return <section className={`settings-page${selectedFile?" settings-page-import-expanded":""}`}>
    {canManageUsers?<UsersPermissions utilities={utilities}/>:<div className="settings-utilities-only">{utilities}</div>}
  </section>;
}
...
function CompactDateRange({ from, to, onFromChange, onToChange, allTime, onAllTime }: { from: string; to: string; onFromChange: (value: string) => void; onToChange: (value: string) => void; allTime: boolean; onAllTime: () => void }) {
  return <div className="compact-date-range"><label><span>من</span><input aria-label="من" type="date" dir="ltr" value={from} onChange={event => onFromChange(event.target.value)} /></label><label><span>إلى</span><input aria-label="إلى" type="date" dir="ltr" value={to} onChange={event => onToChange(event.target.value)} /></label><button type="button" className={allTime ? "soft active" : "soft"} aria-pressed={allTime} onClick={onAllTime}>عرض الكل</button></div>;
}
...
function Pos({
  ...
  const invoice = <FramedSection title="الفاتورة" className="invoice-card workspace-invoice">
    <InvoiceEditorToolbar number={editingDocument ? displayDocumentNumber(editingDocument) : String(data.nextDocumentSequences.sale)} newInvoice={() => { if (!dirty() || confirm("لديك تغييرات غير محفوظة. هل تريد بدء فاتورة جديدة؟")) resetEditor(); }} modes={<><button type="button" className="selection-option" aria-pressed={priceMode === "retail"} onClick={() => changePriceMode("retail")}>بيع الفرد</button><button type="button" className="selection-option" aria-pressed={priceMode === "wholesale"} onClick={() => changePriceMode("wholesale")}>بيع الجملة</button></>} scanner={<BarcodeScanner products={data.products} onScan={add} enabled={scannerEnabled} onEnabledChange={setScannerEnabled} onError={setStockNotice} />} />
...
function CompactPaymentSelector({ accounts, value, onChange }: { accounts: PaymentAccount[]; value: string; onChange: (id: string) => void }) {
  ...
  return <div className="compact-payment" aria-label="طريقة الدفع">
    {cash && <button type="button" className="soft selection-option" aria-pressed={!selectedBank} onClick={() => onChange(cash.id)}>نقدي</button>}
    <label className={selectedBank ? "bank-option selected-bank" : "bank-option"}><span className="sr-only">الحساب البنكي</span><select aria-label="الحساب البنكي" value={selectedBank?.id ?? ""} onChange={event => event.target.value && onChange(event.target.value)}><option value="">بنك</option>{banks.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
  </div>;
}
...
function Expenses({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  const expenseDocs = filterDocumentsByDate(data.documents.filter(d => d.kind === "expense"
  ...
  ), historyFrom, historyTo, historyAllTime);
...
function BankScopeControls({scope}:{scope:ReturnType<typeof useBankScope>}){return <div className="bank-scope-controls"><CompactDateRange from={scope.draftFrom} to={scope.draftTo} allTime={scope.period===null} onAllTime={scope.all} onFromChange={scope.setDraftFrom} onToChange={scope.setDraftTo}/><button className="primary" onClick={scope.commit}>عرض</button></div>}
...
function Banks({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void }) {
  const [tab,setTab]=useState<"accounts"|"movements"|"transfers"|"adjustment">("accounts"),[editing,setEditing]=useState<PaymentAccount|null>(null),[detail,setDetail]=useState<FinancialDetail|null>(null);
  ...
  const active=data.paymentAccounts.filter(account=>account.isActive),name=(id:string)=>data.paymentAccounts.find(account=>account.id===id||account.code===id)?.name??id;
  ...
  const operationalMovements=data.financialMovements.filter(m=>m.type !== "opening-balance"),movements=filterFinancialMovements(operationalMovements,movementScope.period,accountFilter,typeFilter),metrics=bankScopeMetrics(data.paymentAccounts,data.documents,movementScope.period,accountFilter,typeFilter);
  ...
  return <section className="banks-workspace workspace-page"><FramedSection title="البنوك والحسابات" className="bank-panel"><div className="bank-tabs"><button className="selection-option" aria-pressed={tab==="accounts"} onClick={()=>setTab("accounts")}>وسائل الدفع</button><button className="selection-option" aria-pressed={tab==="movements"} onClick={()=>setTab("movements")}>حركة الحسابات</button><button className="selection-option" aria-pressed={tab==="transfers"} onClick={()=>setTab("transfers")}>التحويلات</button><button className="selection-option" aria-pressed={tab==="adjustment"} onClick={()=>setTab("adjustment")}>السحب والإيداع</button></div><div className="bank-tab-stage">
  {tab==="accounts"&&<div className="bank-tab-content bank-tab-accounts"><div className="section-toolbar"><button className="primary" onClick={()=>setEditing({id:"",code:"",name:"",color:"#1677c8",icon:"wallet",isActive:true,balance:0,income:0,expenses:0,purchaseTotal:0})}><Plus/> إضافة بنك أو وسيلة دفع</button></div><div className="erp-table-wrap account-list"><table className="erp-table" aria-label="وسائل الدفع"><thead><tr><th>الحساب</th><th>الحالة</th><th>الرصيد</th><th>إجراء</th></tr></thead><tbody>{data.paymentAccounts.map(account=><tr key={account.id}><td>{account.name}</td><td>{account.isActive?"نشط":"متوقف"}</td><td>{money(account.balance)}</td><td><button className="soft" onClick={()=>setEditing(account)}>تعديل</button></td></tr>)}</tbody></table></div></div>}
  {tab==="movements"&&<div className="bank-tab-content bank-tab-movements"><div className="bank-filter-stack"><div className="bank-filters"><BankScopeControls scope={movementScope}/><select value={accountFilter} onChange={e=>setAccountFilter(e.target.value)}><option value="">كل الحسابات</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={typeFilter} onChange={e=>setTypeFilter(e.target.value)}><option value="">كل الأنواع</option>{Object.entries(movementLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>{noteClauses&&<small className="bank-scope-note">تنبيه: العرض الحالي حسب {noteClauses}</small>}</div><div className="erp-table-wrap ledger-list"><table className="erp-table"><thead><tr><th>التاريخ</th><th>النوع</th><th>الحساب</th><th>الحركة</th><th>المستند</th></tr></thead><tbody>{movements.map(m=><tr key={m.id} onClick={()=>inspectMovement(m)}><td>{formatDateTime(m.occurredAt)}</td><td>{movementLabels[m.type]??m.type}</td><td>{name(m.paymentMethod)}</td><td>{m.direction==="in"?"+":"−"}{number(m.amount)}</td><td dir="ltr">{m.documentNumber}</td></tr>)}</tbody></table></div></div>}
  {tab==="transfers"&&<div className="bank-tab-content bank-tab-transfers"><form className="transfer-form" onSubmit={async e=>{e.preventDefault();await run({type:"account-transfer.post",fromAccountId:transferFrom,toAccountId:transferTo,amount:val(amount),note},"تم التحويل بين الحسابات");setAmount("");setNote("")}}><label>من الحساب<select required value={transferFrom} onChange={e=>setTransferFrom(e.target.value)}><option value="">اختر المصدر</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>إلى الحساب<select required value={transferTo} onChange={e=>setTransferTo(e.target.value)}><option value="">اختر الوجهة</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={amount} onChange={setAmount}/></label><label>ملاحظة<input value={note} onChange={e=>setNote(e.target.value)}/></label><button className="primary" disabled={!transferFrom||!transferTo||transferFrom===transferTo||!amount}>اعتماد التحويل</button></form><div className="bank-history"><div className="bank-filters"><BankScopeControls scope={transferScope}/><select value={transferFromFilter} onChange={e=>setTransferFromFilter(e.target.value)}><option value="">كل المرسلين</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={transferToFilter} onChange={e=>setTransferToFilter(e.target.value)}><option value="">كل المستلمين</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></div><div className="erp-table-wrap transfer-list"><table className="erp-table"><thead><tr><th>التاريخ</th><th>من</th><th>إلى</th><th>المبلغ</th><th>المرجع</th></tr></thead><tbody>{transfers.map(t=><tr key={t.id} onClick={()=>setDetail({type:"تحويل",occurredAt:t.occurredAt,amount:t.amount,reference:t.number,note:t.note,from:name(t.fromAccountId),to:name(t.toAccountId)})}><td>{formatDateTime(t.occurredAt)}</td><td>{name(t.fromAccountId)}</td><td>{name(t.toAccountId)}</td><td>{money(t.amount)}</td><td dir="ltr">{t.number}</td></tr>)}</tbody></table></div></div></div>}
  {tab==="adjustment"&&<div className="bank-tab-content bank-tab-adjustment"><form className="bank-adjustment-form" onSubmit={async e=>{e.preventDefault();await run({type:"account-adjustment.post",accountId:adjustmentAccount,direction:adjustmentDirection,amount:val(adjustmentAmount),note:adjustmentNote},adjustmentDirection==="deposit"?"تم الإيداع":"تم السحب");setAdjustmentAmount("");setAdjustmentNote("")}}><div className="bank-adjustment-direction"><button type="button" className="selection-option" aria-pressed={adjustmentDirection==="deposit"} onClick={()=>setAdjustmentDirection("deposit")}>إيداع</button><button type="button" className="selection-option" aria-pressed={adjustmentDirection==="withdrawal"} onClick={()=>setAdjustmentDirection("withdrawal")}>سحب</button></div><label>الحساب<select required value={adjustmentAccount} onChange={e=>setAdjustmentAccount(e.target.value)}><option value="">اختر الحساب</option>{active.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={adjustmentAmount} onChange={setAdjustmentAmount}/></label><label>ملاحظة<input value={adjustmentNote} onChange={e=>setAdjustmentNote(e.target.value)}/></label><button className="primary">اعتماد العملية</button></form><div className="bank-history"><div className="bank-filters"><BankScopeControls scope={adjustmentScope}/><select value={adjustmentFilter} onChange={e=>setAdjustmentFilter(e.target.value)}><option value="">كل الحسابات</option>{data.paymentAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select><select value={adjustmentType} onChange={e=>setAdjustmentType(e.target.value)}><option value="">إيداع وسحب</option><option value="manual-deposit">إيداع</option><option value="manual-withdrawal">سحب</option></select></div><div className="erp-table-wrap adjustment-list"><table className="erp-table"><thead><tr><th>المرجع</th><th>التاريخ</th><th>الحساب</th><th>النوع</th><th>المبلغ</th><th>الملاحظة</th></tr></thead><tbody>{adjustments.map(m=><tr key={m.id} onClick={()=>inspectMovement(m)}><td dir="ltr">{m.documentNumber}</td><td>{formatDateTime(m.occurredAt)}</td><td>{name(m.paymentMethod)}</td><td>{movementLabels[m.type]}</td><td>{money(m.amount)}</td><td>{m.note||"—"}</td></tr>)}</tbody></table></div></div></div>}
  </div></FramedSection><FramedSection title="ملخص الحسابات" className="bank-summary"><div><small>إجمالي الأرصدة الحالية</small><b>{money(metrics.currentBalance)}</b></div><div><small>إجمالي المصاريف</small><b className="negative">{money(metrics.expenses)}</b></div><div><small>إجمالي المبيعات</small><b className="positive">{money(metrics.sales)}</b></div><div><small>صافي الربح</small><b className={metrics.profit>0?"positive":metrics.profit<0?"negative":""}>{money(metrics.profit)}</b></div></FramedSection>{editing&&<PaymentAccountDialog account={editing} close={()=>setEditing(null)} run={run}/>} {detail&&<FinancialOperationDetail detail={detail} close={()=>setDetail(null)}/>}</section>;
}
...
function PaymentAccountDialog({ account, close, run }: { account: PaymentAccount; close: () => void; run: RunCommand }) {
  ...
  const remove=async()=>{if(account.code==="cash"){alert("لا يمكن حذف وسيلة الدفع النقدية الأساسية");return}const historical=account.income>0||account.expenses>0||account.purchaseTotal>0;const message=historical?"هذه الوسيلة مرتبطة بحركات سابقة.\nسيتم إيقافها عن العمليات الجديدة مع الاحتفاظ بسجلها.":`هل تريد حذف وسيلة الدفع "${account.name}"؟\nلن يمكن التراجع عن الحذف.`;if(!confirm(message))return;await run({type:"payment-account.delete",accountId:account.id},historical?"تم إيقاف الوسيلة مع الاحتفاظ بسجلها":"تم حذف وسيلة الدفع");close()};
...
export function partyBusinessMetrics(documents:DocumentRecord[],partyId:string,partyType:"customer"|"supplier"){
  const kind=partyType==="customer"?"sale":"purchase", docs=documents.filter(document=>document.status==="posted"&&document.kind===kind&&document.partyId===partyId);
  return {total:docs.reduce((sum,document)=>sum+Number(document.total||0),0),grossProfit:partyType==="customer"?docs.reduce((sum,document)=>sum+document.lines.reduce((lineSum,line)=>lineSum+(Number.isFinite(Number(line.grossProfit))?Number(line.grossProfit):0),0),0):null};
}
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  const metrics=list.map(party=>[party.id,partyBusinessMetrics(data.documents,party.id,partyType)] as const), metricMap=new Map(metrics), total=metrics.reduce((sum,item)=>sum+item[1].total,0), profit=customer?metrics.reduce((sum,item)=>sum+Number(item[1].grossProfit??0),0):null;
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    ...
    <div className="party-list-metrics"><span><small>{customer?"إجمالي مبيعات العملاء":"إجمالي مشترياتنا من الموردين"}</small><b>{money(total)}</b></span>{customer&&<span><small>إجمالي الربح من مبيعات العملاء</small><b>{money(profit??0)}</b></span>}</div>
    <FramedSection title={plural} className="parties-list"><div className="erp-table-wrap party-grid"><table className="erp-table" aria-label={plural}><thead><tr><th>رقم</th><th>اسم {noun}</th><th>الهاتف</th><th>{customer?"إجمالي مشترياته":"إجمالي مشترياتنا منه"}</th>{customer&&<th>الربح الإجمالي</th>}<th>الرصيد الحالي</th><th>إجراء</th></tr></thead><tbody>{list.map((party,index)=>{const balance=party.receivable-party.payable,metric=metricMap.get(party.id)!;return <tr key={party.id} onClick={()=>openParty(party)}><td className="num-cell">{number(index+1)}</td><td className="name-cell">{party.name}</td><td dir="ltr">{party.phone||"—"}</td><td className="num-cell">{money(metric.total)}</td>{customer&&<td className="num-cell positive">{money(metric.grossProfit??0)}</td>}<td className={`num-cell ${balance>0?"positive":balance<0?"negative":""}`}>{balance>0?`${money(balance)} لنا`:balance<0?`${money(Math.abs(balance))} علينا`:"متوازن"}</td><td className="action-cell"><button className="soft" onClick={event=>{event.stopPropagation();openParty(party)}}>كشف الحساب</button></td></tr>})}{!list.length&&<tr><td colSpan={customer?7:6}>{customer?"لا يوجد عملاء حتى الآن":"لا يوجد موردون"}</td></tr>}</tbody></table></div></FramedSection>
  </section>;
}
function PartyPage({party,data,openDoc,run}:{party:Party;data:BootstrapData;openDoc:(id:string)=>void;run:RunCommand}) {
  const [from,setFrom]=useState(""),[to,setTo]=useState(""),[amount,setAmount]=useState(""),[paymentMethod,setPaymentMethod]=useState("cash"),[direction,setDirection]=useState<"receive"|"pay">("receive"),[note,setNote]=useState("");
  const customer=resolvePartyType(party)==="customer", balance=party.receivable-party.payable,metrics=partyBusinessMetrics(data.documents,party.id,customer?"customer":"supplier");
  ...
  const submit=async()=>{await run({type:"party-cash.post",partyId:party.id,partyType:resolvePartyType(party),direction,amount:val(amount),paymentMethod,note},"تم تسجيل الحركة");setAmount("");setNote("")};
  return <section className={`party-detail ${customer?"customer-detail":"supplier-detail"}`}><div className="party-detail-top"><FramedSection title={customer?"حساب العميل":"حساب المورد"} className="party-account-summary"><div className="party-identity"><div className="party-identity-field"><small>الاسم</small><strong>{party.name}</strong><small>الهاتف</small><strong><bdi dir="ltr">{party.phone||"—"}</bdi></strong></div><div className="party-trade-metrics"><span><small>{customer?"إجمالي ما اشتراه منا":"إجمالي مشترياتنا منه"}</small><b>{money(metrics.total)}</b></span>{customer&&<span><small>الربح الإجمالي من مبيعاته</small><b>{money(metrics.grossProfit??0)}</b></span>}</div></div><div className={`party-balance ${balance>0?"receivable":balance<0?"payable":"neutral"}`}><span>{balance>0?"مستحق لنا":balance<0?"مستحق علينا":"الحساب متوازن"}</span><strong>{money(Math.abs(balance))}</strong></div></FramedSection><FramedSection title="حركة مالية" className="party-payment-panel"><div className="party-payment-row"><div className="party-cash-direction"><button className="selection-option" aria-pressed={direction==="receive"} onClick={()=>setDirection("receive")}>استلام من {customer?"العميل":"المورد"}</button><button className="selection-option" aria-pressed={direction==="pay"} onClick={()=>setDirection("pay")}>دفع للطرف</button></div><label>الحساب<select value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}>{data.paymentAccounts.filter(a=>a.isActive).map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</select></label><label>المبلغ<Num value={amount} onChange={setAmount}/></label><label>ملاحظة<input value={note} onChange={e=>setNote(e.target.value)}/></label><button className="primary" disabled={!Number.isFinite(val(amount))||val(amount)<=0} onClick={()=>void submit()}>تسجيل الحركة</button></div></FramedSection></div><FramedSection title="سجل الحساب" className="party-history"><div className="party-history-toolbar"><CompactDateRange from={from} to={to} allTime={!from&&!to} onAllTime={()=>{setFrom("");setTo("")}} onFromChange={setFrom} onToChange={setTo}/></div><Recent title="الحركات" docs={docs} openDoc={openDoc} bare/></FramedSection></section>;
}
...
function Warehouses({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  const availableWarehouses=activeWarehouses(data.warehouses), [wh, setWh] = useState(activeWarehouses(data.warehouses)[0]?.id ?? ""), [q, setQ] = useState(""), [browserOpen, setBrowserOpen] = useState(false), [detailProduct, setDetailProduct] = useState<Product | null>(null), [movementFilter, setMovementFilter] = useState("all"), [from,setFrom]=useState(""), [to,setTo]=useState("");
  ...
  const chooseWarehouse = (value: string) => { setWh(value); setQ(""); setDetailProduct(null); };
  return <section className="warehouse-workspace">
    <FramedSection title="المخزن" className="warehouse-head" allowOverflow><label>المخزن النشط<SearchableSelect value={wh} onChange={chooseWarehouse} placeholder="اختر المخزن" searchPlaceholder="ابحث عن مخزن" options={availableWarehouses.map(w => ({ value: w.id, label: w.name }))} /></label><div className="warehouse-actions">{active?.isSalesDefault && <span className="status">مخزن البيع الافتراضي</span>}<button className="soft" disabled={active?.isSalesDefault} onClick={() => void run({ type: "warehouse.default", warehouseId: wh }, "تم تحديد مخزن البيع الافتراضي")}>جعله مخزن البيع الافتراضي</button></div></FramedSection>

    <FramedSection title="جرد المخزن" className={`inventory-panel${browserOpen ? " browser-open" : ""}`}>
      <div className="inventory-overview"><div className="inventory-toolbar"><CompactDateRange from={from} to={to} allTime={!from&&!to} onAllTime={()=>{setFrom("");setTo("")}} onFromChange={setFrom} onToChange={setTo}/><button className="soft" onClick={() => window.print()}><Printer /> طباعة الجرد</button><button className={browserOpen ? "primary active" : "primary"} aria-expanded={browserOpen} onClick={() => { setBrowserOpen(x => !x); if (browserOpen) setDetailProduct(null); }}>{browserOpen ? "إخفاء الجرد" : "عرض الكل"}</button></div>{browserOpen && <div className="inventory-overview-metrics"><span><small>عدد المنتجات</small><b>{number(inventoryProducts.length)}</b></span><span><small>إجمالي الكمية</small><b>{number(totalPieces)}</b></span><span><small>قيمة المخزون</small><b>{number(inventoryValue)}</b></span></div>}</div>
      {browserOpen && <div className="inventory-browser"><div className="inventory-list-panel"><label className="search compact-search"><Search /><input value={q} onChange={e => setQ(e.target.value)} placeholder="ابحث بالاسم أو الكود أو الباركود" /></label><div className="erp-table-wrap warehouse-scroll inventory-body"><table className="erp-table inventory-grid" aria-label="جرد المخزن"><thead><tr><th>رقم</th><th>اسم المنتج</th><th>الباركود</th><th>سعر الشراء</th><th>الكمية الحالية</th><th>الكمية المشتراة</th><th>الكمية المباعة</th><th>قيمة المخزون</th></tr></thead><tbody>{products.map((product, index) => <tr className={detailProduct?.id === product.id ? "selected" : ""} key={product.id} onClick={() => { setDetailProduct(product); setMovementFilter("all"); }}><td className="num-cell">{number(index + 1)}</td><td className="name-cell">{product.name}{isProductExpired(product)&&<small className="expired-badge">منتهي — غير قابل للبيع</small>}</td><td dir="ltr">{product.barcode||"—"}</td><td className="num-cell">{number(inventoryUnitCost(product))}</td><td className="num-cell">{number(qty(product))}</td><td className="num-cell">{number(periodQuantity(data.documents,product.id,wh,"purchase",from,to))}</td><td className="num-cell">{number(periodQuantity(data.documents,product.id,wh,"sale",from,to))}</td><td className="num-cell">{number(qty(product) * inventoryUnitCost(product))}</td></tr>)}{!products.length && <tr><td colSpan={8}>لا توجد منتجات مطابقة للبحث</td></tr>}</tbody></table></div></div></div>}{detailProduct&&createPortal(<div className="modal-overlay" role="dialog" aria-modal="true"><div className="modal-card product-movement-modal"><ProductMovementPanel product={detailProduct} selectedWarehouseId={wh} data={data} filter={movementFilter} setFilter={setMovementFilter} close={()=>setDetailProduct(null)} openDoc={openDoc} from={from} to={to}/></div></div>,document.body)}
    </FramedSection>
  </section>;
}
...
function Products({ data, run }: { data: BootstrapData; run: RunCommand }) {
  ...
  return <section className="workspace-page products-page">
    <div className="toolbar workspace-toolbar">
      ...
      <button className="soft selection-option" aria-pressed={showArchived} onClick={() => setShowArchived(value => !value)}>{showArchived ? "إخفاء المؤرشفة" : "عرض المؤرشفة"}</button>
    ...
    <FramedSection title="قائمة المنتجات" className="scroll-panel product-management">
      ...
      <table className="erp-table" aria-label="كل المنتجات"><colgroup><col style={{width:"10%"}}/><col style={{width:"22%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"11%"}}/><col style={{width:"12%"}}/><col style={{width:"23%"}}/></colgroup><thead><tr>
        <th>رقم</th><th>الاسم</th><th>{sortHeader("price", "سعر البيع")}</th><th>سعر الجملة</th><th>{sortHeader("cost", "آخر شراء")}</th><th>{sortHeader("stock", "المخزون")}</th><th>إجراءات</th>
      </tr></thead><tbody>
  ...
  return (
    <section className="records-workspace">
      <FramedSection title="بحث السجلات" className="records-filters"><div className="filters">
        ...
        <CompactDateRange from={from} to={to} allTime={allTime} onAllTime={() => setAllTime(true)} onFromChange={value => { setFrom(value); setAllTime(false); }} onToChange={value => { setTo(value); setAllTime(false); }} />
      </div></FramedSection>
      <Recent title="كل السجلات القابلة للتتبع" docs={docs} openDoc={openDoc} />
    </section>
  );
}
const reportNames: Record<ReportType,string> = { overview:"التقرير الشامل",sales:"حركة المبيعات",purchases:"حركة المشتريات","product-sales":"حركة المنتجات",stock:"حركة المخزون",profit:"تحليل الأرباح",debts:"الحسابات والديون","party-ledger":"كشف حسابات الأطراف",financial:"الحركة المالية",expenses:"المصاريف",returns:"المرتجعات" };
const reportColumns = (type: ReportType, productId: string, groupBy: string): Array<[string,string]> => ({
 ...
 "product-sales":[["product","اسم المنتج"],["soldQuantity","الكمية المباعة"],["returnedQuantity","الكمية المرتجعة"],["currentQuantity","الكمية الحالية"],["sales","إجمالي المبيعات"],["returns","قيمة مرتجعات البيع"],["netSales","صافي المبيعات"],["purchasedQuantity","الكمية المشتراة"],["purchases","إجمالي المشتريات"],["netPurchases","صافي المشتريات"],["averagePrice","متوسط سعر البيع"],["averagePurchasePrice","متوسط سعر الشراء"],["profit","الربح"]],
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  ...
  const numericKeys=new Set(["quantity","unitPrice","total","cost","profit","margin","paid","due","sales","purchases","expenses","received","net","soldQuantity","returnedQuantity","currentQuantity","purchasedQuantity","netPurchases","returns","netSales","averagePrice","averagePurchasePrice","invoiceCount","before","change","after","incoming","outgoing","receivable","payable","balance","debit","credit","products"]);
  ...
  if(type==="overview") {
    const groups = (["sale","purchase","expense","return"] as const).map(kind => ({ kind, label: kind === "sale" ? "فواتير البيع" : kind === "purchase" ? "فواتير الشراء" : kind === "expense" ? "فواتير المصاريف" : "مرتجعات البيع", rows: (result?.invoices ?? []).filter(invoice => invoice.kind === kind) })).filter(group => group.rows.length);
    const bankTotal=(result?.bankAccounts??[]).reduce((sum,account)=>sum+account.balance,0);
    return <section className="reports-workspace overview-workspace comprehensive-report" onKeyDown={e=>{if(e.key==="Enter"&&e.target===e.currentTarget)void runReport(false)}}><div className="overview-print-title"><strong>Conta</strong><h2>التقرير الشامل</h2><span>{allTimeActive?"الفترة: كل السجلات":`الفترة: من ${formatDate(from)} إلى ${formatDate(to)}`}</span></div><FramedSection title="بيانات التقرير" className="report-toolbar overview-toolbar no-print"><CompactDateRange from={from} to={to} allTime={allTimeActive} onAllTime={()=>void runReport(true)} onFromChange={setFrom} onToChange={setTo}/><button className="primary" onClick={()=>void runReport(false)}>عرض</button><button onClick={()=>window.print()}><Printer/> طباعة</button></FramedSection>{reportError&&<div className="error">{reportError}</div>}{busy&&<div className="report-loading">جاري إعداد التقرير…</div>}{result?<div className="overview-grid"><FramedSection title="الحسابات" className="overview-accounts"><div className="erp-table-wrap"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم الحساب</th><th>نوع الحساب</th><th>مستحق له</th><th>مستحق عليه</th></tr></thead><tbody>{(result.parties??[]).map((p,i)=><tr key={String(p.id)}><td>{number(i+1)}</td><td>{p.name}</td><td>{p.partyType==="customer"?"عميل":"مورد"}</td><td className="num-cell">{p.partyType==="supplier"?number(reportNumber(p.payable)):"—"}</td><td className="num-cell">{p.partyType==="customer"?number(reportNumber(p.receivable)):"—"}</td></tr>)}</tbody></table></div></FramedSection><FramedSection title="الفواتير" className="overview-invoices"><div className="erp-table-wrap"><table className="erp-table overview-invoice-table"><thead><tr><th>رقم</th><th>نوع الفاتورة</th><th>التاريخ</th><th>قيمة الفاتورة</th><th>التكلفة</th><th>الربح</th></tr></thead><tbody>{groups.map(group=><Fragment key={group.kind}><tr className="invoice-group-row"><th colSpan={6}>{group.label}</th></tr>{group.rows.map((d,i)=><tr key={d.id} onClick={()=>openDoc(d.documentId)}><td>{number(i+1)}</td><td>{d.type}</td><td>{formatDate(d.occurredAt)}</td><td className="num-cell">{number(d.invoiceValue)}</td><td className="num-cell">{number(d.cost)}</td><td className={`num-cell ${d.profit===null?"":d.profit>0?"positive":d.profit<0?"negative":""}`}>{d.profit===null?"—":number(d.profit)}</td></tr>)}</Fragment>)}</tbody></table></div><div className="overview-business-strip">{[["sales","إجمالي المبيعات"],["salesCost","تكلفة المبيعات"],["salesProfit","ربح المبيعات"],["purchases","إجمالي المشتريات"],["expenses","إجمالي المصاريف"]].map(([key,label])=><span key={key}><small>{label}</small><b className={key==="salesProfit"?(reportNumber(result.summary[key])>0?"positive":reportNumber(result.summary[key])<0?"negative":""):""}>{number(reportNumber(result.summary[key]))}</b></span>)}</div></FramedSection><FramedSection title="الذمم والحسابات · الأرصدة الحالية" className="overview-statement">{[["supplierPayables","إجمالي المستحق للموردين علينا","negative"],["customerReceivables","إجمالي المستحق لنا من العملاء","positive"],["supplierCount","عدد الموردين",""],["customerCount","عدد العملاء",""]].map(([key,label,tone])=><div key={key}><span>{label}</span><b className={tone}>{number(reportNumber(result.summary[key]))}</b></div>)}</FramedSection><FramedSection title="الأرصدة الحالية" className="overview-statement overview-current-balances"><div><span>إجمالي تكلفة المخزون الحالية</span><b>{number(reportNumber(result.summary.inventoryValue))}</b></div><div className="comprehensive-bank-disclosure"><button type="button" aria-expanded={bankOpen} onClick={()=>setBankOpen(open=>!open)}><span>إجمالي أرصدة البنوك الحالية</span><b>{number(bankTotal)} <ChevronDown/></b></button>{bankOpen&&<div className="bank-detail no-print">{(result.bankAccounts??[]).map(account=><span key={account.id}><span>{account.name}</span><b className={account.balance<0?"negative":""}>{number(account.balance)}</b></span>)}<strong><span>الإجمالي</span><b>{number(bankTotal)}</b></strong></div>}<div className="bank-detail print-only"><h3>أرصدة البنوك الحالية</h3>{(result.bankAccounts??[]).map(account=><span key={account.id}><span>{account.name}</span><b>{number(account.balance)}</b></span>)}<strong><span>الإجمالي</span><b>{number(bankTotal)}</b></strong></div></div></FramedSection></div>:<div className="overview-empty">اختر الفترة ثم اضغط عرض</div>}</section>;
  }
  return <section className="reports-workspace" onKeyDown={e=>{if(e.key==="Enter")void runReport(false)}}><FramedSection title="بيانات البحث" className="report-toolbar no-print">{showDates&&<CompactDateRange from={from} to={to} allTime={allTimeActive} onAllTime={()=>void runReport(true)} onFromChange={setFrom} onToChange={setTo}/>}<button className="primary" onClick={()=>void runReport(false)}>عرض</button><button onClick={()=>window.print()}><Printer/> طباعة</button><div className="report-filters">
