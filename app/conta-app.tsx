    ...
    <div className={`app-shell section-${view}`} dir="rtl">
      <aside className={menu ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label="التنقل الرئيسي">
          ...
          {can("banks.view")&&<div className="nav-menu bank-nav-menu" ref={bankMenuRef}><button className={view==="banks"?"nav active":"nav"} aria-expanded={bankMenu} onClick={()=>setBankMenu(open=>!open)}><Landmark/><span>البنوك</span><ChevronDown className="chevron"/></button>{bankMenu&&<div className="nav-popover bank-nav-popover">{bankNav.map(item=><button key={item.id} className={bankTab===item.id?"active":""} onClick={()=>{setBankTab(item.id);navigate("banks")}}><span>{item.label}</span></button>)}</div>}</div>}
...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  const [editing,setEditing]=useState<PaymentAccount|null>(null),[detail,setDetail]=useState<FinancialDetail|null>(null);
  ...
  return <section className="banks-workspace workspace-page"><FramedSection title={bankNav.find(item=>item.id===tab)?.label??"البنوك والحسابات"} className="bank-panel"><div className="bank-tab-stage">
...
function Warehouses({ data, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  const [draftFrom,setDraftFrom]=useState(""),[draftTo,setDraftTo]=useState(""),[committedPeriod,setCommittedPeriod]=useState<CommittedPeriod>(null),[periodError,setPeriodError]=useState("");
  ...
  const commitPeriod=()=>{if(draftFrom&&draftTo&&draftFrom>draftTo){setPeriodError("تاريخ البداية يجب ألا يتجاوز تاريخ النهاية");return}setPeriodError("");setCommittedPeriod({from:draftFrom,to:draftTo})};
...
function Reports({ data, openDoc, type }: { data: BootstrapData; openDoc: (id: string) => void; type: ReportType }) {
  const now=new Date(),[draftFrom,setDraftFrom]=useState(`${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,"0")}-01`),[draftTo,setDraftTo]=useState(now.toISOString().slice(0,10)),[committedPeriod,setCommittedPeriod]=useState<CommittedPeriod>(()=>({from:`${now.getUTCFullYear()}-${String(now.getUTCMonth()+1).padStart(2,"0")}-01`,to:now.toISOString().slice(0,10)})),[partyId,setPartyId]=useState(""),[productId,setProductId]=useState(""),[accountId,setAccountId]=useState(""),[groupBy,setGroupBy]=useState("invoice"),[sortState,setSortState]=useState<{key:string;direction:"asc"|"desc"}|null>(null),[movementType,setMovementType]=useState(""),[direction,setDirection]=useState(""),[debtSide,setDebtSide]=useState(""),[search,setSearch]=useState(""),[result,setResult]=useState<ReportResponse|null>(null),[busy,setBusy]=useState(false),[reportError,setReportError]=useState("");
  const [bankOpen,setBankOpen]=useState(false), [partyTypeFilter,setPartyTypeFilter]=useState<"customer"|"supplier">("customer");
  const reportRequest=useRef<AbortController|null>(null);
  const runReport=async(period:CommittedPeriod)=>{if(type==="party-ledger"&&!partyId){setReportError(partyTypeFilter==="customer"?"اختر العميل أولاً":"اختر المورد أولاً");return}reportRequest.current?.abort();const controller=new AbortController();reportRequest.current=controller;setBusy(true);setReportError("");const q=new URLSearchParams({type,unpaged:"true"});if(type!=="debts")Object.entries(reportDateQuery(period===null,period?.from??"",period?.to??"")).forEach(([key,value])=>q.set(key,value));const add=(key:string,value:string)=>{if(value)q.set(key,value)};if(["sales","purchases","product-sales","profit","returns","stock"].includes(type))add("productId",productId);if(type==="party-ledger")add("partyId",partyId);if(["sales","purchases","financial","expenses"].includes(type))add("paymentAccountId",accountId);if(type==="profit")add("groupBy",groupBy);if(type==="stock")add("movementType",movementType);if(type==="financial")add("direction",direction);if(type==="debts"){add("debtSide",debtSide);add("search",search)}try{const response=await fetch(`/api/reports?${q}`,{signal:controller.signal}),json=await response.json();if(!response.ok)throw new Error(json.error);if(!controller.signal.aborted)setResult(json)}catch(error){if((error as Error).name!=="AbortError")setReportError(error instanceof Error?error.message:"تعذر إنشاء التقرير")}finally{if(reportRequest.current===controller)setBusy(false)}};
  ...
  const applyAllTime=()=>{setCommittedPeriod(null);void runReport(null)};
