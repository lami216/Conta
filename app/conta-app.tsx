...
function Banks({ data, run, openDoc, tab }: { data: BootstrapData; run: RunCommand; openDoc:(id:string)=>void; tab:BankTab }) {
  ...
  const operationalMovements=data.financialMovements.filter(m=>m.type !== "opening-balance"),movements=filterFinancialMovements(operationalMovements,movementScope.period,accountFilter,typeFilter),accountSummary=bankScopeMetrics(data.paymentAccounts,data.documents,null);
  ...
  {tab==="accounts"&&<div className="bank-tab-content bank-tab-accounts"><div className="bank-accounts-main"><div className="section-toolbar"><button className="primary" onClick={()=>setEditing({id:"",code:"",name:"",color:"#1677c8",icon:"wallet",isActive:true,balance:0,income:0,expenses:0,purchaseTotal:0})}><Plus/> إضافة بنك أو وسيلة دفع</button></div><div className="erp-table-wrap account-list"><table className="erp-table" aria-label="وسائل الدفع"><colgroup><col style={{width:"34%"}}/><col style={{width:"19%"}}/><col style={{width:"27%"}}/><col style={{width:"20%"}}/></colgroup><thead><tr><th>الحساب</th><th>الحالة</th><th>الرصيد</th><th>إجراء</th></tr></thead><tbody>{data.paymentAccounts.map(account=><tr key={account.id}><td>{account.name}</td><td>{account.isActive?"نشط":"متوقف"}</td><td className={`num-cell ${account.balance>0?"metric-positive":account.balance<0?"metric-negative":"metric-neutral"}`}>{money(account.balance)}</td><td className="action-cell"><button className="soft" onClick={()=>setEditing(account)}>تعديل</button></td></tr>)}</tbody></table></div></div><FramedSection title="ملخص الحسابات" className="bank-summary"><div className={accountSummary.currentBalance>0?"metric-positive":accountSummary.currentBalance<0?"metric-negative":"metric-neutral"}><small>إجمالي الأرصدة الحالية</small><b>{money(accountSummary.currentBalance)}</b></div><div className="metric-neutral"><small>إجمالي المبيعات</small><b>{money(accountSummary.sales)}</b></div><div className="metric-negative"><small>إجمالي المصاريف</small><b>{money(accountSummary.expenses)}</b></div><div className={accountSummary.profit>0?"metric-positive":accountSummary.profit<0?"metric-negative":"metric-neutral"}><small>صافي الربح</small><b>{money(accountSummary.profit)}</b></div></FramedSection></div>}
  ...
  </div></FramedSection>{editing&&<PaymentAccountDialog account={editing} close={()=>setEditing(null)} run={run}/>} {detail&&<FinancialOperationDetail detail={detail} close={()=>setDetail(null)}/>}</section>;
}
...
function Parties({ partyType, data, run, openParty }: { partyType: "customer" | "supplier"; data: BootstrapData; run: RunCommand; openParty: (p: Party) => void }) {
  ...
  const summaries=new Map(data.partyFinancialSummaries.map(summary=>[summary.partyId,summary])),metrics=list.map(party=>[party.id,partyTradeMetrics(summaries.get(party.id),partyType)] as const), metricMap=new Map(metrics), total=metrics.reduce((sum,item)=>sum+item[1].total,0), profit=customer?metrics.reduce((sum,item)=>sum+Number(item[1].grossProfit??0),0):null;
  return <section className={`${customer?"customer":"supplier"}-workspace parties-workspace`}>
    ...
    <FramedSection title={plural} className="parties-list"><div className="erp-table-wrap party-grid"><table className="erp-table" aria-label={plural}><thead><tr><th>رقم</th><th>اسم {noun}</th><th>الهاتف</th><th>{customer?"إجمالي مشترياته":"إجمالي مشترياتنا منه"}</th>{customer&&<th>الربح الإجمالي</th>}<th>الرصيد الحالي</th><th>إجراء</th></tr></thead><tbody>{list.map((party,index)=>{const balance=party.receivable-party.payable,metric=metricMap.get(party.id)!;return <tr key={party.id} onClick={()=>openParty(party)}><td className="num-cell">{number(index+1)}</td><td className="name-cell">{party.name}</td><td dir="ltr">{party.phone||"—"}</td><td className="num-cell">{money(metric.total)}</td>{customer&&<td className={`num-cell ${Number(metric.grossProfit??0)>0?"metric-positive":Number(metric.grossProfit??0)<0?"metric-negative":"metric-neutral"}`}>{money(metric.grossProfit??0)}</td>}<td className={`num-cell ${balance>0?"positive":balance<0?"negative":""}`}>{balance>0?`${money(balance)} لنا`:balance<0?`${money(Math.abs(balance))} علينا`:"متوازن"}</td><td className="action-cell"><button className="soft" onClick={event=>{event.stopPropagation();openParty(party)}}>كشف الحساب</button></td></tr>})}{!list.length&&<tr><td colSpan={customer?7:6}>{customer?"لا يوجد عملاء حتى الآن":"لا يوجد موردون"}</td></tr>}</tbody></table></div></FramedSection>
    <div className="party-list-metrics"><span className="metric-neutral"><small>{customer?"إجمالي مبيعات العملاء":"إجمالي مشترياتنا من الموردين"}</small><b>{money(total)}</b></span>{customer&&<span className={Number(profit??0)>0?"metric-positive":Number(profit??0)<0?"metric-negative":"metric-neutral"}><small>إجمالي الربح من مبيعات العملاء</small><b>{money(profit??0)}</b></span>}</div>
  </section>;
}
