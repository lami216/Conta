...
function SearchableSelect({ value, onChange, options, placeholder, searchPlaceholder, disabled = false, allowEmpty = false, floating = false, variant = "normal", ariaLabel }: {
  ...
  placeholder: string; searchPlaceholder: string; disabled?: boolean; allowEmpty?: boolean; floating?: boolean; variant?: "normal" | "compact"; ariaLabel?: string;
}) {
  ...
  const position = useCallback(() => {
    ...
    const rect = root.current.getBoundingClientRect(), margin = 8, desiredHeight = Math.min(variant === "compact" ? 235 : 330, window.innerHeight - margin * 2);
    const below = window.innerHeight - rect.bottom - margin, above = rect.top - margin, opensUp = below < 220 && above > below;
    const width = Math.min(Math.max(rect.width, variant === "compact" ? 220 : 280), variant === "compact" ? 300 : window.innerWidth - margin * 2, window.innerWidth - margin * 2);
  ...
  const list = <div ref={popover} className={`combobox-popover${floating ? " combobox-popover-floating" : ""}${variant === "compact" ? " combobox-popover-compact" : ""}`} style={floating ? floatingStyle : undefined}>
  ...
  return <div className={`combobox${variant === "compact" ? " combobox-compact" : ""}`} ref={root}>
...
function Pos({
  ...
  const customerSelect = <SearchableSelect value={partyId} onChange={setPartyId} placeholder={payment === "note" ? "اختر العميل" : "بيع مباشر"} searchPlaceholder="ابحث باسم أو هاتف" floating allowEmpty={payment !== "note"} variant="compact" ariaLabel="العميل" options={data.parties.filter(p => resolvePartyType(p) === "customer").map(p => ({ value: p.id, label: p.name, search: p.phone }))} />;
...
function CompactPaymentSelector({ accounts, value, onChange }: { accounts: PaymentAccount[]; value: string; onChange: (id: string) => void }) {
  ...
  return <div className="compact-payment" aria-label="طريقة الدفع">
    ...
    <label className={selectedBank ? "bank-option selected-bank" : "bank-option"}><span className="sr-only">الحساب البنكي</span><select value={selectedBank?.id ?? ""} onChange={event => onChange(event.target.value)}><option value="">بنك ▼</option>{banks.map(account => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
  </div>;
}
