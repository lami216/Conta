...
function Pos({
  ...
  const invoice = <div className="panel invoice-card workspace-invoice">
    ...
    <div className={lines.length ? "invoice-preview has-items" : "invoice-preview"}>{lines.length ? <div className="invoice-preview-list" role="table" aria-label="منتجات الفاتورة"><div className="invoice-table-row invoice-table-head" role="row"><span>الاسم</span><span>الكمية</span><span>السعر</span><span>المجموع</span><span>حذف</span></div>{details.map(({ l, p, total: lineTotal }) => <div className={`invoice-preview-item invoice-table-row${selectedLine === p.id ? " selected" : ""}`} role="row" tabIndex={0} aria-selected={selectedLine === p.id} onClick={() => setSelectedLine(p.id)} key={p.id}><span className="invoice-item-name"><b>{p.name}</b></span><span><Num value={l.quantity} onChange={value => updateSaleLine(p, { quantity: value })} /></span><span><Num value={l.piecePrice} onChange={value => updateSaleLine(p, { piecePrice: value })} /></span><strong className="invoice-item-total">{money(lineTotal)}</strong><span className="invoice-delete-cell"><button type="button" className="row-delete" aria-label={`حذف ${p.name}`} onClick={event => { event.stopPropagation(); setLines(current => current.filter(item => item.productId !== p.id)); }}><X /></button></span></div>)}</div> : <div className="empty-invoice-state"><span><ReceiptText /></span><b>الفاتورة فارغة</b><small>أضف المنتجات لبدء فاتورة جديدة</small></div>}</div>
  </div>;
...
function Purchases({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  return <section className="transaction-page"><div className="transaction-workspace purchase-workspace">
    ...
    <div className="panel invoice-card workspace-invoice"><div className="invoice-card-head"><h3>فاتورة الشراء الحالية</h3><div><span className="product-count">{number(lines.length)} منتج</span>{lines.length > 0 && <button className="clear-draft" onClick={clearDraft}>مسح الفاتورة</button>}</div></div><div className={lines.length ? "invoice-preview has-items" : "invoice-preview"}>{lines.length ? <div className="invoice-preview-list" role="table"><div className="invoice-table-row invoice-table-head"><span>الاسم</span><span>الكمية</span><span>سعر الشراء</span><span>المجموع</span><span>حذف</span></div>{details.map(({line, product}) => <div key={product.id} tabIndex={0} onClick={() => setSelectedLine(product.id)} className={`invoice-preview-item invoice-table-row${selectedLine === product.id ? " selected" : ""}`}><span className="invoice-item-name"><b>{product.name}</b></span><span><Num value={line.quantity} onChange={value => updatePurchaseLine(product, { quantity: value })} /></span><span><Num value={line.unitPrice} onChange={value => updatePurchaseLine(product, { unitPrice: value })} /></span><strong className="invoice-item-total">{money(val(line.quantity) * val(line.unitPrice))}</strong><span className="invoice-delete-cell"><button type="button" className="row-delete" aria-label={`حذف ${product.name}`} onClick={event => { event.stopPropagation(); setLines(current => current.filter(item => item.productId !== product.id)); }}><X /></button></span></div>)}</div> : <div className="empty-invoice-state"><span><ReceiptText /></span><b>الفاتورة فارغة</b><small>يمكن إضافة عدة منتجات قبل الاعتماد</small></div>}</div></div>
...
function Expenses({ data, run, openDoc }: { data: BootstrapData; run: RunCommand; openDoc: (id: string) => void }) {
  ...
  return <section className="expense-workspace workspace-page">
    <div className="expense-grid">
      ...
      <div className="panel expense-recurring"><div className="section-title"><h3>المصاريف المستحقة</h3><b>{number(data.recurringExpenses.length)}</b></div><div className="expense-scroll">
