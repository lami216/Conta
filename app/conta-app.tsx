...
function UsersPermissions() {
  ...
  return <FramedSection title="المستخدمون والصلاحيات" className="users-permissions">
    <div className="users-permissions-layout">
      <fieldset className="permissions-pane"><legend>صلاحيات المستخدم</legend><div className="permission-toolbar"><span>اختر الصلاحيات الفعلية المطلوبة</span><button className="soft" type="button" onClick={()=>setPermissions(everythingSelected?[]:[...allPermissions])}>{everythingSelected?"إلغاء الكل":"تحديد الكل"}</button></div><div className="permissions-grid"><table className="erp-table"><thead><tr><th>رقم</th><th>اسم الشاشة</th><th>الصلاحيات</th><th>تحكم كامل</th></tr></thead><tbody>{permissionRows.map((row,i)=>{const actions=Object.entries(row.actions) as Array<[PermissionAction,string]>,keys=actions.map(([,key])=>key),full=keys.every(key=>permissions.includes(key));return <tr key={row.name}><td>{number(i+1)}</td><td>{row.name}</td><td><div className="permission-actions">{actions.map(([action,key])=><label key={key}><input aria-label={`${row.name} ${actionLabels[action]}`} type="checkbox" checked={permissions.includes(key)} onChange={event=>setPermissions(setPermission(permissions,key,event.target.checked))}/><span>{actionLabels[action]}</span></label>)}</div></td><td><input aria-label={`${row.name} تحكم كامل`} type="checkbox" checked={full} onChange={event=>setPermissions(setRowFullControl(permissions,keys,event.target.checked))}/></td></tr>})}</tbody></table></div></fieldset>
...
function SettingsPage({data,reload}:{data:BootstrapData;reload:()=>Promise<void>}) {
  ...
  const importExternal=async()=>{if(!externalPreview?.uploadId)return;setBusy("import");setFailure("");try{let run=await readApiResponse(await fetch("/api/settings/legacy/upload/complete",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({uploadId:externalPreview.uploadId,action:"import",stockPolicy,accountBalancePolicy:accountPolicy,filename:selectedFile?.name})})) as ImportRun;setImportRun(run);run=await advance(run);setMessage(`تم الدمج بأمان. نسخة الرجوع: ${run.backupIdBeforeImport}`);await reload()}catch(e){setFailure(e instanceof Error?e.message:"تعذر الاستيراد")}finally{setBusy("")}};
    ...
    <div className="settings-utility-row"><FramedSection title="النسخ الاحتياطي" className="settings-backup"><div><button className="primary" disabled={!!busy} onClick={()=>{setBusy("backup");download().then(()=>setMessage("تم إنشاء النسخة وتنزيلها")).catch(e=>setFailure(e.message)).finally(()=>setBusy(""))}}>{busy==="backup"?"جاري الإنشاء…":"إنشاء وتنزيل"}</button></div></FramedSection>
