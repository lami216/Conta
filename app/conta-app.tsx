...
function PartyAggregateMetrics({partyType,metrics}:{partyType:"customer"|"supplier";metrics:ReturnType<typeof partyAggregateMetrics>}) {
  const customer=partyType==="customer", cards=customer
    ? [["إجمالي ما اشتراه العملاء منا",metrics.tradeTotal,"neutral"],["إجمالي ما دفعه العملاء لنا",metrics.cashIn,"positive"],["إجمالي ما دفعناه للعملاء",metrics.cashOut,"negative"],["إجمالي الربح من مبيعات العملاء",metrics.grossProfit,metrics.grossProfit>0?"positive":metrics.grossProfit<0?"negative":"neutral"]] as const
    : [["إجمالي مشترياتنا من الموردين",metrics.tradeTotal,"neutral"],["إجمالي ما دفعناه للموردين",metrics.cashOut,"negative"],["إجمالي ما دفعه الموردون لنا",metrics.cashIn,"positive"],["عدد فواتير الشراء",metrics.invoiceCount,"count"]] as const;
  return <div className="party-list-metrics">{cards.map(([label,value,tone])=><span key={label} className={`metric-${tone==="count"?"neutral":tone}`}><small>{label}</small>{tone==="count"?<b>{number(value)}</b>:<MoneyValue value={value} tone={tone==="positive"?"positive":tone==="negative"?"negative":"neutral"} className="financial-amount-summary"/>}</span>)}</div>;
}
