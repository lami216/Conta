...
async function execute(db: Db, session: ClientSession, body: Input) {
  ...
  if (type === "product.create" || type === "product.update") {
    ...
    const values = { name, sku, barcode, pieceCost, piecePrice: optionalPrice(body.piecePrice), cartonPrice: optionalPrice(body.cartonPrice), piecesPerCarton };
  ...
  if (["payment.post", "settlement.post", "offset.post"].includes(type)) {
    ...
    const kind = type.split(".")[0], doc = { ...baseDocument(kind, kind === "offset" ? "OFF" : kind === "payment" ? "PAY" : "SET"), partyId, partyName: party.name, warehouseId: null, warehouseName: null, destinationWarehouseId: null, destinationWarehouseName: null, parentDocumentId: null, paymentMethod: text(body.paymentMethod) || "cash", title: null, total: requested, dueTotal: 0, paidTotal: requested, lines: [] }; await db.collection("parties").updateOne({ id: partyId }, { $set: { receivable, payable, net: receivable - payable } }, { session }); await db.collection("documents").insertOne(doc, { session }); return doc.id;
  }
  if (type === "expense.post") { const title = text(body.title), amount = positive(body.amount, "المبلغ"), occurredAt = text(body.occurredAt); if (!title || !/^\d{4}-\d{2}-\d{2}$/.test(occurredAt)) throw new CommandError("العنوان والتاريخ مطلوبان"); const doc = { ...baseDocument("expense", "EXP"), occurredAt: new Date(`${occurredAt}T12:00:00Z`).toISOString(), partyId: null, partyName: null, warehouseId: null, warehouseName: null, destinationWarehouseId: null, destinationWarehouseName: null, parentDocumentId: null, paymentMethod: "cash", title, total: amount, dueTotal: 0, paidTotal: amount, lines: [{ id: id("line"), productId: null, description: title, quantity: 1, unitPrice: amount, lineTotal: amount }] }; await db.collection("documents").insertOne(doc, { session }); const frequency = text(body.frequency); if (["daily", "monthly"].includes(frequency)) await db.collection("recurringExpenses").insertOne({ id: id("rec"), title, amount, frequency, startsOn: occurredAt, active: true, createdAt: new Date() }, { session }); return doc.id; }
  if (type === "expense.materialize") { const recurringId = text(body.recurringId), dueDate = text(body.dueDate), recurring = await db.collection("recurringExpenses").findOne({ id: recurringId, active: true }, { session }); if (!recurring) throw new CommandError("المصروف المتكرر غير موجود", 404); const existing = await db.collection("documents").findOne({ recurringId, dueDate }, { session }); if (existing) return String(existing.id); const doc = { ...baseDocument("expense", "EXP"), occurredAt: new Date(`${dueDate}T12:00:00Z`).toISOString(), recurringId, dueDate, partyId: null, partyName: null, warehouseId: null, warehouseName: null, destinationWarehouseId: null, destinationWarehouseName: null, parentDocumentId: null, paymentMethod: "cash", title: recurring.title, total: recurring.amount, dueTotal: 0, paidTotal: recurring.amount, lines: [{ id: id("line"), productId: null, description: recurring.title, quantity: 1, unitPrice: recurring.amount, lineTotal: recurring.amount }] }; await db.collection("documents").insertOne(doc, { session }); return doc.id; }
