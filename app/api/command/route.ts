...
export async function execute(db: Db, session: ClientSession, body: Input) {
  ...
  if (["sale.update", "purchase.update"].includes(type)) {
    ...
    if (isSale && await db.collection("documents").findOne({ kind: "return", status: "posted", parentDocumentId: documentId }, { session })) throw new CommandError("لا يمكن تعديل أو حذف هذه الفاتورة لوجود مرتجع مرتبط بها.", 409);
  ...
  if (type === "payment-account.update") { const accountId = text(body.id), name = text(body.name), color=text(body.color); if (!name) throw new CommandError("بيانات وسيلة الدفع غير صالحة"); const result = await db.collection("paymentAccounts").updateOne({ id: accountId }, { $set: { name, ...(color&&/^#[0-9a-f]{6}$/i.test(color)?{color}:{}), isActive: body.isActive !== false, updatedAt: new Date() } }, { session }); if (!result.matchedCount) throw new CommandError("وسيلة الدفع غير موجودة", 404); return accountId; }
