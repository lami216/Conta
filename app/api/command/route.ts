...
export async function POST(request: Request) {
  ...
  try {
    ...
    const db = await getMongo(), client = getMongoClient(); let result: unknown = "";
    await client.withSession(session => session.withTransaction(async () => { await db.collection("auditEvents").insertOne({ id: id("audit"), action: type, status: "started", createdAt: new Date() }, { session }); result = await execute(db, session, body); await db.collection("auditEvents").insertOne({ id: id("audit"), action: type, entityId: typeof result === "object" && result ? (result as {id?:unknown}).id : result, status: "committed", createdAt: new Date() }, { session }); }, { readConcern: { level: "snapshot" }, writeConcern: { w: "majority" } }));
    log("info", "api.command.completed", { commandType: type, entityId: result }); return Response.json(typeof result === "object" && result ? result : { id: result });
