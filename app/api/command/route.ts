...
export async function execute(db: Db, session: ClientSession, body: Input) {
  ...
  if (type === "payment-account.update") { const accountId = text(body.id), name = text(body.name), color = text(body.color), icon = text(body.icon); if (!name || !/^#[0-9a-f]{6}$/i.test(color) || !["banknote", "wallet", "building", "landmark", "card"].includes(icon)) throw new CommandError("بيانات وسيلة الدفع غير صالحة"); const result = await db.collection("paymentAccounts").updateOne({ id: accountId }, { $set: { name, color, icon, isActive: body.isActive !== false, updatedAt: new Date() } }, { session }); if (!result.matchedCount) throw new CommandError("وسيلة الدفع غير موجودة", 404); return accountId; }
  if (type === "payment-account.create") { const name = text(body.name), color = text(body.color), icon = text(body.icon); if (!name || !/^#[0-9a-f]{6}$/i.test(color) || !["banknote", "wallet", "building", "landmark", "card"].includes(icon)) throw new CommandError("بيانات وسيلة الدفع غير صالحة"); const account = { id: id("account"), code: id("custom"), name, color, icon, isActive: true, balance: 0, createdAt: new Date() }; await db.collection("paymentAccounts").insertOne(account, { session }); return account.id; }
