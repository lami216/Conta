...
export async function buildReport(db: Db, f: ReportFilters): Promise<ReportResponse> {
  ...
  if (f.type === "sales") {
    ...
    const summaryFacts = await saleFacts(db, [...sales.all, ...returns], f), totals = profitSummary(summaryFacts),pageIds=new Set(sales.rows.map(document=>String(document.id))),pageFacts=summaryFacts.filter(fact=>pageIds.has(String(fact.documentId)));
  ...
  if (f.type === "party-ledger") {
    ...
    const movementLabel = (d: Document) => d.kind === "sale" ? "فاتورة بيع" : d.kind === "purchase" ? "فاتورة شراء" : d.kind === "return" ? (parentKinds.get(String(d.parentDocumentId)) === "purchase" ? "مرتجع شراء" : "مرتجع بيع") : d.kind === "offset" ? "مقاصة" : d.kind === "settlement" ? "تسوية" : d.partyCashDirection === "receive" ? `استلام من ${role}` : d.partyCashDirection === "pay" ? (partyType === "customer" ? "دفع للعميل" : "دفع للمورد") : "دفعة";
    ...
    const net = n(party.receivable) - n(party.payable);
    return { report: f.type, from: f.from ?? null, to: f.to ?? null, summary: { name: String(party.name), partyType, tradeTotal, receivable: Math.max(net, 0), payable: Math.max(-net, 0), net, transactionCount: total }, rows, meta: pagination(total, f) };
  }
  ...
  if(f.type==="product-sales"){
    ...
    const productQuery:Document={isArchived:{$ne:true},...(f.productId?{id:f.productId}:{})};
    ...
    for(const product of productRows)map.set(String(product.id),{id:String(product.id),productId:String(product.id),sku:String(product.sku??"—")||"—",product:String(product.name??"").trim()||"منتج غير متاح",soldQuantity:0,returnedQuantity:0,currentQuantity:Object.entries((product.stocks??{}) as Record<string,unknown>).filter(([warehouseId])=>activeWarehouses.has(warehouseId)).reduce((sum,[,quantity])=>sum+n(quantity),0),sales:0,returns:0,netSales:0,purchasedQuantity:0,purchases:0,netPurchases:0,averagePrice:0,averagePurchasePrice:0,profit:0,costKnown:true});
    for(const fact of facts){const key=String(fact.productId),row=map.get(key);if(!row)continue;if(n(fact.quantity)>=0){row.soldQuantity=n(row.soldQuantity)+n(fact.quantity);row.sales=n(row.sales)+n(fact.revenue)}else{row.returnedQuantity=n(row.returnedQuantity)+Math.abs(n(fact.quantity));row.returns=n(row.returns)+Math.abs(n(fact.revenue))}row.netSales=n(row.sales)-n(row.returns);row.averagePrice=n(row.soldQuantity)?n(row.sales)/n(row.soldQuantity):0;row.costKnown=Boolean(row.costKnown)&&Boolean(fact.costKnown);row.profit=n(row.profit)+n(fact.profit)}
  ...
  const [parties,accounts,products]=await Promise.all([db.collection("parties").find().sort({name:1}).toArray(),db.collection("paymentAccounts").find({isActive:true,code:{$ne:"cash"}}).sort({createdAt:1,name:1}).toArray(),db.collection("products").find({isArchived:{$ne:true}}).project({stocks:1,lastPurchaseCost:1,pieceCost:1}).toArray()]);
  ...
  const kindRank:Record<string,number>={sale:0,purchase:1,expense:2,return:3};
  const invoices=commercial.map(d=>{const kind=String(d.kind) as "sale"|"purchase"|"expense"|"return",value=n(d.total),saleFact=factsByDocument.get(String(d.id));return{id:String(d.id),documentId:String(d.id),kind,type:kind==="sale"?"فاتورة بيع":kind==="purchase"?"فاتورة شراء":kind==="expense"?"فاتورة مصروفات":"إرجاع بيع",number:displayDocumentNumber(d),sequence:Number.isSafeInteger(Number(d.sequence))&&n(d.sequence)>0?n(d.sequence):null,occurredAt:String(d.occurredAt),invoiceValue:value,cost:kind==="sale"?(saleFact?.cost??0):value,profit:kind==="sale"?(saleFact?.profit??value):null}}).sort((a,b)=>kindRank[a.kind]-kindRank[b.kind]||((a.sequence??Number.MAX_SAFE_INTEGER)-(b.sequence??Number.MAX_SAFE_INTEGER))||a.occurredAt.localeCompare(b.occurredAt)||a.id.localeCompare(b.id));
