...
export async function buildReport(db: Db, f: ReportFilters): Promise<ReportResponse> {
  ...
  const partyRows=typedParties.map(p=>({id:String(p.id),partyId:String(p.id),name:String(p.name),partyType:p.partyType,receivable:p.partyType==="customer"?n(p.receivable):0,payable:p.partyType==="supplier"?n(p.payable):0}));
  ...
  return{report:"overview",from:f.from!,to:f.to!,summary:{sales,salesCost:p.cost,salesProfit:p.profit,purchases:commercial.filter(d=>d.kind==="purchase").reduce((v,d)=>v+n(d.total),0),expenses:commercial.filter(d=>d.kind==="expense").reduce((v,d)=>v+n(d.total),0),profit:p.profit,bankBalance,inventoryValue:products.reduce((v,product)=>v+Object.values((product.stocks??{}) as Record<string,unknown>).reduce<number>((q,value)=>q+n(value),0)*(Number.isFinite(product.lastPurchaseCost)?n(product.lastPurchaseCost):Number.isFinite(product.pieceCost)?n(product.pieceCost):0),0),customerReceivables:typedParties.filter(p=>p.partyType==="customer").reduce((v,p)=>v+n(p.receivable),0),supplierPayables:typedParties.filter(p=>p.partyType==="supplier").reduce((v,p)=>v+n(p.payable),0),customerCount:typedParties.filter(p=>p.partyType==="customer").length,supplierCount:typedParties.filter(p=>p.partyType==="supplier").length},rows:[],invoices,parties:partyRows,bankAccounts,meta:pagination(invoices.length,f)};
