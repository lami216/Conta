...
export async function buildReport(db: Db, f: ReportFilters): Promise<ReportResponse> {
  ...
  if (f.type === "sales") {
    ...
    const rows = f.productId ? await saleFacts(db, sales.rows, f) : await Promise.all(sales.rows.map(async document => { const facts = await saleFacts(db, [document], f), p = profitSummary(facts); return { id: String(document.id), documentId: String(document.id), number: String(document.number), occurredAt: String(document.occurredAt), party: String(document.partyName ?? "بيع مباشر"), paymentMethod: String(document.paymentMethod ?? ""), total: n(document.total), cost: p.cost, profit: p.profit, margin: n(document.total) ? p.profit / n(document.total) * 100 : 0, paid: n(document.paidTotal), due: n(document.dueTotal) }; }));
  ...
  if (f.type === "purchases" || f.type === "expenses" || f.type === "returns") {
    ...
    const identities = new Map((await db.collection("products").find({ id: { $in: productIds } }).project({ id: 1, name: 1, sku: 1 }).toArray()).map(product => [String(product.id), product]));
    const rows = found.rows.map(document => { const selected = ((document.lines ?? []) as Document[]).filter(line => lineMatches(line, f)); if (f.type === "returns") return { id: String(document.id), documentId: String(document.id), occurredAt: String(document.occurredAt), number: String(document.number), originalDocument: originals.get(String(document.parentDocumentId)) ?? "غير متاحة", party: String(document.partyName ?? ""), products: selected.length, quantity: selected.reduce((s,l)=>s+n(l.quantity),0), total: selected.reduce((s,l)=>s+n(l.lineTotal),0) }; if (f.type === "purchases" && f.productId) { const line=selected[0]; return { id:String(document.id),documentId:String(document.id),number:String(document.number),occurredAt:String(document.occurredAt),party:String(document.partyName??""),product:String(identities.get(String(line?.productId))?.name??line?.description??"").trim()||"منتج غير متاح",sku:String(identities.get(String(line?.productId))?.sku??line?.sku??"—")||"—",quantity:n(line?.quantity),unitPrice:n(line?.unitPrice),total:n(line?.lineTotal) }; } return { id:String(document.id),documentId:String(document.id),number:String(document.number),occurredAt:String(document.occurredAt),party:String(document.partyName??""),paymentMethod:String(document.paymentMethod??""),title:String(document.title??""),recurring:Boolean(document.recurringId),total:n(document.total),paid:n(document.paidTotal),due:n(document.dueTotal) }; });
  ...
  const commercial=await db.collection("documents").find({kind:{$in:["sale","return","purchase","expense"]},status:"posted",...matchDate(f)}).toArray(),financial=await db.collection("financialMovements").find({type:{$ne:"legacy-opening-balance"},...matchDate(f)}).toArray(),byDay=new Map<string,ReportRow>();for(const d of commercial){const day=String(d.occurredAt).slice(0,10),r=byDay.get(day)??{date:day,sales:0,purchases:0,expenses:0,received:0,paid:0,net:0};if(d.kind==="sale")r.sales=n(r.sales)+n(d.total);if(d.kind==="return")r.sales=n(r.sales)-n(d.total);if(d.kind==="purchase")r.purchases=n(r.purchases)+n(d.total);if(d.kind==="expense")r.expenses=n(r.expenses)+n(d.total);byDay.set(day,r)}for(const movement of financial.filter(x=>!String(x.type).startsWith("transfer-"))){const day=String(movement.occurredAt).slice(0,10),r=byDay.get(day)??{date:day,sales:0,purchases:0,expenses:0,received:0,paid:0,net:0};if(movement.direction==="in")r.received=n(r.received)+n(movement.amount);else r.paid=n(r.paid)+n(movement.amount);r.net=n(r.received)-n(r.paid);byDay.set(day,r)}const rows=[...byDay.values()].sort((a,b)=>String(a.date).localeCompare(String(b.date)));const p=profitSummary(facts);return{report:"overview",from:f.from!,to:f.to!,summary:{sales:commercial.filter(d=>d.kind==="sale").reduce((s,d)=>s+n(d.total),0),returns:commercial.filter(d=>d.kind==="return").reduce((s,d)=>s+n(d.total),0),purchases:commercial.filter(d=>d.kind==="purchase").reduce((s,d)=>s+n(d.total),0),expenses:commercial.filter(d=>d.kind==="expense").reduce((s,d)=>s+n(d.total),0),received:financial.filter(x=>!String(x.type).startsWith("transfer-")&&x.direction==="in").reduce((s,x)=>s+n(x.amount),0),paid:financial.filter(x=>!String(x.type).startsWith("transfer-")&&x.direction==="out").reduce((s,x)=>s+n(x.amount),0),expiredInventoryLoss:expiryLoss,cashFlow:financial.filter(x=>!String(x.type).startsWith("transfer-")).reduce((s,x)=>s+(x.direction==="in"?n(x.amount):-n(x.amount)),0),profit:p.profit,unknownRevenue:p.unknownRevenue},rows:slice(rows,f),meta:pagination(rows.length,f)};
}
