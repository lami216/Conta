...
test("overview daily aggregation and summaries do not depend on page size or page",async()=>{await db.dropDatabase();await db.collection("financialMovements").insertMany(Array.from({length:205},(_,i)=>({id:String(i),occurredAt:`2026-08-${String(1+i%20).padStart(2,"0")}T12:00:00Z`,type:"sale",direction:"in",amount:1})));const one=await buildReport(db,filters("overview",{page:1,pageSize:5})),two=await buildReport(db,filters("overview",{page:2,pageSize:5}));assert.equal(one.summary.received,205);assert.deepEqual(one.summary,two.summary);assert.equal(one.meta.totalRows,20);});
